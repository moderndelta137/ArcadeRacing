...
export default function ArcadeRacingGame() {
  ...
  useEffect(() => {
    ...
    postOnlineMessage({
      ...
      selection: { selectedCourseId, courseDirection, randomMapSize, selectedCars, activeCarPlayer }
    });
  }, [gameMode, appState, onlineSession.roomKey, onlineSession.role, selectedCourseId, courseDirection, randomMapSize, selectedCars, activeCarPlayer]);
  ...
  return (
    ...
    >
      ...
      {/* Fanned Cards UI */}
      {activeHandVisible && (
        <div className={`absolute ${activeHandTucked ? '-bottom-28' : 'bottom-8'} left-[55%] -translate-x-1/2 flex items-end justify-center z-40 w-full no-pan transition-all duration-300`} style={{ perspective: '1200px' }}>
          ...
          {activeHandCanPlay && <button onClick={skipToDiscard} className="absolute -bottom-2 right-[10%] px-6 py-3 bg-red-950/80 hover:bg-red-900 border border-red-900 rounded-lg text-sm font-bold text-red-200 transition-colors shadow-2xl">Skip Turn</button>}
        </div>
      )}
