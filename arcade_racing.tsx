...
const TERRAIN_BACKDROP_PADDING = TERRAIN_BACKDROP_SIZE / 2;
const RACE_LANE_WIDTH = 70;
const RACE_SPACE_LENGTH = 140;
const BACKGROUND_ASSET_DIR = 'Assets/Backgrounds';
const COURSE_BACKGROUNDS = {
  akagi: `${BACKGROUND_ASSET_DIR}/touge_topdown_forest_02.png`,
  haruna: `${BACKGROUND_ASSET_DIR}/snow_field_topdown_06.png`,
  hakone: `${BACKGROUND_ASSET_DIR}/touge_topdown_night_forest_03.png`
};
const WILDCARD_BACKGROUNDS = [
  `${BACKGROUND_ASSET_DIR}/snow_field_topdown_06.png`,
  `${BACKGROUND_ASSET_DIR}/tokyo_night_buildings_05.png`,
  `${BACKGROUND_ASSET_DIR}/touge_topdown_forest_02.png`,
  `${BACKGROUND_ASSET_DIR}/touge_topdown_night_forest_03.png`,
...
const slugifyCardImageName = (name) => String(name || '')
  .trim()
  .toLowerCase()
  .replace(/[^a-z0-9]+/g, '-')
  .replace(/^-+|-+$/g, '');
...
const getCourseBackground = (courseId) => {
  if (courseId === 'random') {
    return WILDCARD_BACKGROUNDS[Math.floor(Math.random() * WILDCARD_BACKGROUNDS.length)];
  }
  return COURSE_BACKGROUNDS[courseId] || COURSE_BACKGROUNDS.akagi;
};
...
function generateTrack(seed = Date.now(), cornerCardRows = DEFAULT_CORNER_CARD_ROWS, retry = 0, requestedRoadCards = 12) {
  ...
  const LANE_W = RACE_LANE_WIDTH;
  const SPACE_LEN = RACE_SPACE_LENGTH;
...
const getTrackBounds = (track) => {
  const spaces = [...(track?.[0] || []), ...(track?.[1] || [])];
  if (!spaces.length) {
    return { center: { x: 0, y: 0 }, minX: 0, maxX: 0, minY: 0, maxY: 0 };
  }

  const points = spaces.flatMap(space => {
    const tokens = (space.path.match(/[MLAZ]|-?\d+(?:\.\d+)?/g) || []);
    const pathPoints = [];

    for (let i = 0; i < tokens.length;) {
      const command = tokens[i++];
      if (command === 'M' || command === 'L') {
        pathPoints.push({ x: Number(tokens[i++]), y: Number(tokens[i++]) });
      } else if (command === 'A') {
        i += 5;
        pathPoints.push({ x: Number(tokens[i++]), y: Number(tokens[i++]) });
      } else if (command !== 'Z') {
    ...
    return [space.center, ...pathPoints];
  });
...
const buildDebugRoadCardTrack = (card, cardIndex) => {
  let cx = 0, cy = 0, dir = 0;
  let globalDistance = 0;
  const LANE_W = RACE_LANE_WIDTH;
  const SPACE_LEN = RACE_SPACE_LENGTH;
  ...
  const getP = (baseX, baseY, angleDeg, offset) => {
    const r = angleDeg * Math.PI / 180;
    return {
      x: baseX + Math.cos(r) * offset,
      y: baseY + Math.sin(r) * offset
    };
  };

  const addNodesStraight = (len, limitL = null, limitR = null) => {
    const startL = leftTrack.length;
    const startR = rightTrack.length;
    ...
    for (let i = 0; i < len; i++) {
      ...
      leftTrack.push({
        id: `debug-${card.id}-L${leftTrack.length}`,
        path: `M ${bl_L.x} ${bl_L.y} L ${br_L.x} ${br_L.y} L ${fr_L.x} ${fr_L.y} L ${fl_L.x} ${fl_L.y} Z`,
        center: centerL,
        limit: limitL,
        adj: [startR + i],
        angle: dir,
      ...
      const centerR = getP(cx + fwdX * ((i + 0.5) * SPACE_LEN), cy + fwdY * ((i + 0.5) * SPACE_LEN), dir, LANE_W / 2);
      rightTrack.push({
        id: `debug-${card.id}-R${rightTrack.length}`,
        path: `M ${br_L.x} ${br_L.y} L ${br_R.x} ${br_R.y} L ${fr_R.x} ${fr_R.y} L ${fr_L.x} ${fr_L.y} Z`,
        center: centerR,
        limit: limitR,
        adj: [startL + i],
        angle: dir,
  ...
  const addNodesCurve = (turnRight, angleDeg, inSpaces, outSpaces, inLimit, outLimit, guardrail, radius = 350) => {
    const sign = turnRight ? 1 : -1;
    const rotCx = cx + Math.cos(dir * Math.PI / 180) * sign * radius;
    const rotCy = cy + Math.sin(dir * Math.PI / 180) * sign * radius;
    const totalSweep = sign * angleDeg;
    const rotStartAngle = turnRight ? dir + 180 : dir;
    const lSpaces = turnRight ? outSpaces : inSpaces;
    const rSpaces = turnRight ? inSpaces : outSpaces;
    ...
    const getArcPath = (r1, r2, stepIdx, totalSteps) => {
      const a1 = rotStartAngle + (stepIdx / totalSteps) * totalSweep;
      const a2 = rotStartAngle + ((stepIdx + 1) / totalSteps) * totalSweep;
    ...
    for (let i = 0; i < lSpaces; i++) {
      ...
      const d = globalDistance + ((i + 0.5) / lSpaces) * curveLenCenter;
      let effLimitL = lLimitVal;
      if (turnRight && i < lSpaces - rSpaces) effLimitL = null;
      leftTrack.push({
        id: `debug-${card.id}-L${leftTrack.length}`,
        path: getArcPath(l_r1, l_r2, i, lSpaces),
        center: { x: rotCx + midR * Math.cos(midA * Math.PI / 180), y: rotCy + midR * Math.sin(midA * Math.PI / 180) },
        limit: effLimitL,
        guardrail: guardrail === 'outer' && turnRight,
        angle: dir + ((i + 0.5) / lSpaces) * totalSweep,
        isOuter: turnRight,
        distance: d,
        adj: []
      });
    ...
    for (let i = 0; i < rSpaces; i++) {
      ...
      const d = globalDistance + ((i + 0.5) / rSpaces) * curveLenCenter;
      let effLimitR = rLimitVal;
      if (!turnRight && i < rSpaces - lSpaces) effLimitR = null;
      rightTrack.push({
        id: `debug-${card.id}-R${rightTrack.length}`,
        path: getArcPath(r_r1, r_r2, i, rSpaces),
        center: { x: rotCx + midR * Math.cos(midA * Math.PI / 180), y: rotCy + midR * Math.sin(midA * Math.PI / 180) },
        limit: effLimitR,
        guardrail: guardrail === 'outer' && !turnRight,
        angle: dir + ((i + 0.5) / rSpaces) * totalSweep,
        isOuter: !turnRight,
        distance: d,
        adj: []
      });
  ...
  const addCard = () => {
    const startLeft = leftTrack.length;
    addNodesStraight(card.entrySpaces);
    let turnRight = true;
    card.segments.forEach(segment => {
      if (segment.type === 'straight') {
        addNodesStraight(segment.spaces);
        return;
      }
      if (segment.direction === 'opposite') turnRight = !turnRight;
      addNodesCurve(turnRight, segment.angle, segment.in_spaces, segment.out_spaces, segment.speed_limit, segment.speed_limit, 'outer', segment.radius);
    });
    addNodesStraight(card.exitSpaces);
    if (leftTrack[startLeft]) leftTrack[startLeft].roadCardStart = { index: cardIndex + 1, name: card.name };
  };

  addCard();
  [...leftTrack, ...rightTrack].forEach(space => {
    space.roadCard = card.name;
    space.roadCardIndex = cardIndex;
  });
  return { 0: leftTrack, 1: rightTrack };
};

const buildRoadDebugLayout = (cards) => {
  const builtCards = cards.map((card, index) => {
    const track = buildDebugRoadCardTrack(card, index);
  ...
  const columns = Math.min(4, Math.max(1, builtCards.length));
  const margin = 520;
  const maxWidth = Math.max(...builtCards.map(item => item.width), 1);
  const maxHeight = Math.max(...builtCards.map(item => item.height), 1);
  const cellWidth = maxWidth + margin;
  const cellHeight = maxHeight + margin;

  const groups = builtCards.map((item, index) => {
    const col = index % columns;
    const row = Math.floor(index / columns);
    return {
      ...item,
      offset: {
        x: col * cellWidth - item.bounds.center.x,
        y: row * cellHeight - item.bounds.center.y
      }
    };
  });

  const allBounds = groups.flatMap(group => ([
    { x: group.bounds.minX + group.offset.x, y: group.bounds.minY + group.offset.y },
    { x: group.bounds.maxX + group.offset.x, y: group.bounds.maxY + group.offset.y }
  ]));
  const bounds = allBounds.reduce((acc, point) => ({
    minX: Math.min(acc.minX, point.x),
    maxX: Math.max(acc.maxX, point.x),
    minY: Math.min(acc.minY, point.y),
    maxY: Math.max(acc.maxY, point.y)
...
const DEFAULT_CARD_ROWS = [
  { id: 'drift', name: 'Drift', category: 'Common / Starter Cards - Bright Red AE86', requirement: 'Max 70', type: 'Turn', timing: 'Drive', effect: '+30 km/h to the speed check this turn.', implemented: 'true', deck_code: 'AE86', serial_number: 'AE86-01' },
  ...
  { id: 'early_brake_cornering', name: 'Early Brake Cornering', category: 'Balance set / Starter Cards - Bright Red AE86', requirement: 'Max 110', type: 'Turn', timing: 'Before', effect: 'Speed -20 km/h. Move to the inner lane.', implemented: 'true', deck_code: 'AE86', serial_number: 'AE86-06' },
  { id: 'rocket_start', name: 'Rocket Start', category: 'Balance set / Starter Cards - Bright Red AE86', requirement: 'Max 50', type: 'Gas', timing: 'After', effect: 'Speed +50 km/h. You may discard 2 cards.', implemented: 'true', deck_code: 'AE86', serial_number: 'AE86-07' },
...
const CAR_OPTIONS = [
  { id: 'ae86', name: 'Toyota AE86', drive: 'RWD', specialty: 'Lightweight balance. Predictable drifts and fast line changes.', portrait: 'Assets/Car Mugshots/toyota-ae86-mugshot.png', cutout: 'Assets/Car Mugshots/Cutouts/toyota-ae86-cutout-v3.png' },
  { id: 'huracan', name: 'Lamborghini Huracan', drive: 'AWD', specialty: 'Explosive exits. Strong grip when power comes down early.', portrait: 'Assets/Car Mugshots/lamborghini-huracan-mugshot.png', cutout: 'Assets/Car Mugshots/Cutouts/lamborghini-huracan-cutout-v3.png' },
  { id: 'porsche911', name: 'Porsche 911', drive: 'RWD', specialty: 'Rear-engine traction. Precise corrections through technical bends.', portrait: 'Assets/Car Mugshots/porsche-911-mugshot.png', cutout: 'Assets/Car Mugshots/Cutouts/porsche-911-cutout-v3.png' },
...
const LANGUAGES = [
  ...
  { code: 'ja', labelKey: 'language.japanese', shortLabel: '日本語' }
];

const buildTranslationTable = (rows) => {
  const table = {};
  rows.forEach(row => {
    if (!row.key) return;
    table[row.key] = row;
...
const translateFromTable = (table, language, key, fallback = key, vars = {}) => {
  const row = table?.[key];
  const value = row?.[language] || row?.en || fallback;
  return interpolateText(value, vars);
};

const getRequirementRule = (requirement, t = null) => {
  const match = requirement.match(/^(Min|Max)\s+(\d+)/i);
  if (!match) return { label: t ? t('card.requirement.none', 'None') : 'None', canPlay: () => true };
  const value = Number(match[2]);
  if (match[1].toLowerCase() === 'min') return { label: t ? t('card.requirement.min', `Min ${value} km/h`, { value }) : `Min ${value} km/h`, canPlay: player => player.speed >= value };
  return { label: t ? t('card.requirement.max', `Max ${value} km/h`, { value }) : `Max ${value} km/h`, canPlay: player => player.speed <= value };
};
...
const getUndersteerTarget = (lane, idx, track) => {
  const current = track[lane][idx];
  const outLane = 1 - lane;
  const adjacent = current.adj.filter(adjIdx => track[outLane]?.[adjIdx]);
  const sortByNearestAhead = (a, b) => track[outLane][a].distance - track[outLane][b].distance || a - b;
  const ahead = adjacent
    .filter(adjIdx => track[outLane][adjIdx].distance >= current.distance)
    .sort(sortByNearestAhead)[0];
  const laneAhead = track[outLane]
    .map((space, laneIdx) => ({ space, laneIdx }))
    .filter(({ space }) => space.distance >= current.distance && space.roadCardIndex === current.roadCardIndex)
    .map(({ laneIdx }) => laneIdx)
    .sort(sortByNearestAhead)[0] ?? track[outLane]
    .map((space, laneIdx) => ({ space, laneIdx }))
    .filter(({ space }) => space.distance >= current.distance)
    .map(({ laneIdx }) => laneIdx)
    .sort(sortByNearestAhead)[0];
  const fallback = adjacent
    .sort((a, b) => track[outLane][b].distance - track[outLane][a].distance)[0];

  return { lane: outLane, idx: ahead ?? laneAhead ?? fallback ?? idx };
};

const getEffectiveSpeedCheck = (player, space) => {
  if (!space || space.limit === null || player.modifiers.ignoreSpeedLimits) return null;

  let effectiveLimit = space.limit;
  if (player.modifiers.driftBonus) effectiveLimit += player.modifiers.driftBonus;
  if (player.modifiers.innerLaneSpeedCheckBonus && !space.isOuter) effectiveLimit += player.modifiers.innerLaneSpeedCheckBonus;
  if (player.modifiers.lineLockBonus && player.modifiers.lineLockLane === player.lane) effectiveLimit += player.modifiers.lineLockBonus;
  if (player.modifiers.speedCheckBonusOnce) effectiveLimit += player.modifiers.speedCheckBonusOnce;

  const effectiveSpeed = player.modifiers.trailBraking && player.speed > effectiveLimit && player.speed - 10 >= 0
    ? player.speed - 10
    : player.speed;

  return { effectiveLimit, effectiveSpeed };
};

const willMoveUndersteer = (player, lane, idx, track) => {
  const space = track[lane]?.[idx];
  const speedCheck = getEffectiveSpeedCheck({ ...player, lane }, space);
  return Boolean(speedCheck && speedCheck.effectiveSpeed > speedCheck.effectiveLimit);
};

const getRoadsideBadgeOffset = (player, track, offset = 106) => {
  const space = track[player.lane]?.[player.idx];
  if (!space) return { x: player.lane === 0 ? -offset : offset, y: -6 };

  const otherLane = 1 - player.lane;
  const adjacentIdx = [...(space.adj || [])]
    .filter(idx => track[otherLane]?.[idx])
    .sort((a, b) => Math.abs(track[otherLane][a].distance - space.distance) - Math.abs(track[otherLane][b].distance - space.distance))[0];

  if (adjacentIdx === undefined) return { x: player.lane === 0 ? -offset : offset, y: -6 };

  const adjacentSpace = track[otherLane][adjacentIdx];
  const angleRad = (space.angle || 0) * Math.PI / 180;
  const localRightX = Math.cos(angleRad);
  const localRightY = Math.sin(angleRad);
  const adjacentSide = Math.sign(
    (adjacentSpace.center.x - space.center.x) * localRightX +
    (adjacentSpace.center.y - space.center.y) * localRightY
  ) || 1;

  return { x: -adjacentSide * offset, y: -6 };
};
...
const buildCardCatalog = (rows, t = null) => {
  ...
  return Object.fromEntries(IMPLEMENTED_CARD_IDS.map(id => {
    ...
    const requirement = getRequirementRule(row.requirement, t);
    ...
    const card = {
      id: row.id,
      name: t ? t(`card.${row.id}.name`, row.name) : row.name,
      ...
      rulesTiming: row.timing,
      req: requirement.label,
      desc: t ? t(`card.${row.id}.effect`, row.effect) : row.effect,
      rulesDesc: row.effect,
...
const RenderCard = ({ card, scale = 1, isHovered = false, onClick, onMouseEnter, onMouseLeave, disabled = false, extraClasses = "", isSelected = false }) => {
  ...
  const shadowColor = rulesType === 'Gas' ? 'rgba(253,224,71,0.8)' : rulesType === 'Brake' ? 'rgba(239,68,68,0.8)' : 'rgba(59,130,246,0.8)';
  const textColor = rulesType === 'Gas' ? 'text-yellow-50' : rulesType === 'Brake' ? 'text-red-50' : 'text-blue-50';
  const typeColor = rulesType === 'Gas' ? 'text-yellow-500' : rulesType === 'Brake' ? 'text-red-500' : 'text-blue-500';
  const [spriteFailed, setSpriteFailed] = useState(false);
  const hasSprite = Boolean(card.imageSrc && !spriteFailed);
  ...
  if (hasSprite) {
    return (
      <button
        ...
        className={`relative h-64 w-[11.5rem] overflow-hidden rounded-xl border-2 bg-black p-0 shadow-2xl transition-colors duration-300
        ${disabled ? 'border-zinc-900 opacity-60 cursor-not-allowed grayscale' : 'border-zinc-500 hover:border-white cursor-pointer'}
      ...
      >
        ...
        />
        {isSelected && <div className="absolute inset-0 rounded-xl bg-red-500/20 pointer-events-none mix-blend-overlay"></div>}
      </button>
    );
  }
...
const buildRoadCardGeometry = (segments, width = 160, height = 120, padding = 14, entrySpaces = 0, exitSpaces = 0) => {
  const laneWidth = 28;
  ...
  const pointAt = (baseX, baseY, angle, forward, lateral) => ({
    x: baseX + Math.cos(angle) * forward + Math.cos(angle + Math.PI / 2) * lateral,
    y: baseY + Math.sin(angle) * forward + Math.sin(angle + Math.PI / 2) * lateral
  });

  const arcPoint = (centerX, centerY, radius, angle) => ({
    x: centerX + Math.cos(angle) * radius,
    y: centerY + Math.sin(angle) * radius
  });

  const makePath = (points) => points.map((point, index) => `${index === 0 ? 'M' : 'L'} ${point.x.toFixed(1)} ${point.y.toFixed(1)}`).join(' ') + ' Z';

  const addStraight = (spaces) => {
    for (let i = 0; i < spaces; i++) {
      const start = i * straightLength;
      const end = (i + 1) * straightLength;
      [
        [-laneWidth, 0],
        [0, laneWidth]
      ].forEach(([innerOffset, outerOffset]) => {
        const points = addPoints([
          pointAt(x, y, heading, start, innerOffset),
          pointAt(x, y, heading, start, outerOffset),
          pointAt(x, y, heading, end, outerOffset),
          pointAt(x, y, heading, end, innerOffset)
        ]);
        cells.push({ points, limit: null, isOuterExtra: false });
      });
    }
    x += Math.cos(heading) * spaces * straightLength;
    y += Math.sin(heading) * spaces * straightLength;
  };

  const addCurveLane = (centerX, centerY, startAngle, sweep, direction, count, innerRadius, outerRadius, limit, showExtraAsOpen, signSide = 'outer') => {
    const steps = Math.max(3, Math.ceil(Math.abs(sweep) / (Math.PI / 18)));
    for (let i = 0; i < count; i++) {
      const a0 = startAngle + direction * sweep * (i / count);
      const a1 = startAngle + direction * sweep * ((i + 1) / count);
      const outerPoints = [];
      const innerPoints = [];
      for (let step = 0; step <= steps; step++) {
        const angle = a0 + (a1 - a0) * (step / steps);
        outerPoints.push(arcPoint(centerX, centerY, outerRadius, angle));
        innerPoints.push(arcPoint(centerX, centerY, innerRadius, angle));
      }
      const points = addPoints(direction > 0
        ? [...innerPoints, ...outerPoints.slice().reverse()]
        : [...outerPoints, ...innerPoints.slice().reverse()]);
      const isOuterExtra = showExtraAsOpen && i < showExtraAsOpen;
      cells.push({ points, limit: isOuterExtra ? null : limit, isOuterExtra });
      if (!isOuterExtra && limit) {
        const midAngle = (a0 + a1) / 2;
        const signOffset = signSide === 'inner' ? innerRadius - 16 : outerRadius + 14;
        signs.push({
          x: centerX + Math.cos(midAngle) * signOffset,
          y: centerY + Math.sin(midAngle) * signOffset,
  ...
  addStraight(Math.max(0, Number(entrySpaces || 0)));

  segments.forEach(segment => {
    if (segment.type === 'straight') {
      addStraight(Math.max(1, Number(segment.spaces || 1)));
      return;
    }

    const direction = segment.direction === 'opposite' ? -lastTurn : segment.direction === 'same' ? lastTurn : lastTurn;
    lastTurn = direction;
    const sweep = (Number(segment.angle || 45) * Math.PI) / 180;
    const radius = Math.max(44, Math.min(92, Number(segment.radius || 350) * 0.16));
    const inSpaces = Math.max(1, Number(segment.in_spaces || 1));
    const outSpaces = Math.max(1, Number(segment.out_spaces || inSpaces));
    const limit = Number(segment.speed_limit || 0);
    const centerX = x + Math.cos(heading + direction * Math.PI / 2) * radius;
    const centerY = y + Math.sin(heading + direction * Math.PI / 2) * radius;
    const startAngle = heading - direction * Math.PI / 2;
    const extraOuterSpaces = Math.max(0, outSpaces - inSpaces);

    if (direction > 0) {
      addCurveLane(centerX, centerY, startAngle, sweep, direction, outSpaces, radius, radius + laneWidth, limit, extraOuterSpaces, 'outer');
      addCurveLane(centerX, centerY, startAngle, sweep, direction, inSpaces, radius - laneWidth, radius, limit, 0, 'inner');
    } else {
      addCurveLane(centerX, centerY, startAngle, sweep, direction, inSpaces, radius - laneWidth, radius, limit, 0, 'inner');
      addCurveLane(centerX, centerY, startAngle, sweep, direction, outSpaces, radius, radius + laneWidth, limit, extraOuterSpaces, 'outer');
    }

    const endAngle = startAngle + direction * sweep;
    x = centerX + Math.cos(endAngle) * radius;
    y = centerY + Math.sin(endAngle) * radius;
    heading += direction * sweep;
  });

  addStraight(Math.max(0, Number(exitSpaces || 0)));

  const bounds = [...allPoints, ...signs].reduce((acc, point) => ({
    minX: Math.min(acc.minX, point.x),
    maxX: Math.max(acc.maxX, point.x),
    minY: Math.min(acc.minY, point.y),
    maxY: Math.max(acc.maxY, point.y)
  }), { minX: Infinity, maxX: -Infinity, minY: Infinity, maxY: -Infinity });
  const roadWidth = Math.max(1, bounds.maxX - bounds.minX);
  const roadHeight = Math.max(1, bounds.maxY - bounds.minY);
  const scale = Math.min((width - padding * 2) / roadWidth, (height - padding * 2) / roadHeight);
  const offsetX = (width - roadWidth * scale) / 2 - bounds.minX * scale;
  const offsetY = (height - roadHeight * scale) / 2 - bounds.minY * scale;
  ...
  return {
    cells: cells.map(cell => ({ ...cell, d: makePath(cell.points.map(fit)) })),
    signs: signs.map(sign => ({ ...sign, ...fit(sign), size: Math.max(8, 13 * scale) }))
  };
};

const RoadSegmentSvg = ({ segments, entrySpaces = 0, exitSpaces = 0, viewWidth = 160, viewHeight = 120, className = "h-32 w-full", showSigns = true }) => {
  const geometry = buildRoadCardGeometry(segments || [], viewWidth, viewHeight, 14, entrySpaces, exitSpaces);

  return (
    <svg viewBox={`0 0 ${viewWidth} ${viewHeight}`} className={className} aria-hidden="true">
      {geometry.cells.map((cell, index) => (
        <path
          key={`cell-${index}`}
          d={cell.d}
          fill={cell.limit ? '#a80f14' : '#242426'}
          stroke={cell.limit ? '#f59e0b' : '#a1a1aa'}
          strokeWidth="1.6"
          strokeDasharray={cell.limit ? undefined : '5 3'}
          strokeLinejoin="round"
        />
      ))}
      {showSigns && geometry.signs.map((sign, index) => (
        <g key={`sign-${index}`} transform={`translate(${sign.x} ${sign.y}) rotate(${sign.angle})`}>
          <line x1="0" y1="0" x2="0" y2={sign.size * 0.9} stroke="#a1a1aa" strokeWidth="2" />
          <circle r={sign.size} fill="#f8fafc" stroke="#dc2626" strokeWidth="3" />
          <text fill="#111827" fontSize={sign.size * 0.95} fontWeight="900" fontFamily="sans-serif" textAnchor="middle" dy={sign.size * 0.34}>{sign.limit}</text>
        </g>
      ))}
    </svg>
  );
...
const RoadCardPreview = ({ card }) => {
  const segments = card?.segments || [];

  return (
    <div className="relative h-64 w-[11.5rem] overflow-hidden rounded-xl border-2 border-zinc-500 bg-[#10130d] p-2 shadow-2xl">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_16%,rgba(250,204,21,0.2),transparent_34%),linear-gradient(145deg,rgba(24,24,27,0.9),rgba(5,5,5,0.96))]" />
      <div className="absolute inset-0 opacity-30 bg-[repeating-linear-gradient(135deg,rgba(255,255,255,0.1)_0px,rgba(255,255,255,0.1)_2px,transparent_2px,transparent_18px)]" />
      <RoadSegmentSvg segments={segments} entrySpaces={card.entrySpaces} exitSpaces={card.exitSpaces} viewWidth={184} viewHeight={256} className="relative h-full w-full drop-shadow-[0_0_18px_rgba(250,204,21,0.18)]" />
    </div>
  );
};

const ONLINE_SESSION_INITIAL = {
  role: null,
  roomKey: '',
  guestInput: '',
...
const TUTORIAL_STEPS = [
  {
    ...
    target: 'active-car',
    title: 'Race Start',
    body: 'This is the full race view. Your goal is simple: cross the finish before your rival. The driver currently in front is the Lead Driver, which matters because the lead usually acts first each turn. If someone overtakes, that order can change.',
    trigger: ({ gameState, canControlActivePlayer }) => canControlActivePlayer && gameState.phase === 'PLAY_CARD' && gameState.logs?.some(log => log.includes('Race started'))
  },
  {
    id: 'speedometer',
    target: 'speedometer-active',
    title: 'Speed, Gear, and MP',
    body: 'The speedometer is your main status panel. Speed is how fast your car is moving, Gear is the speed band you are in, and MP means Move Points. Higher speed gives you a higher gear and more MP, but it also makes tight corners more dangerous.',
    trigger: ({ gameState, canControlActivePlayer }) => canControlActivePlayer && gameState.phase === 'PLAY_CARD' && gameState.tutorial?.seen?.['turn-flow']
  },
  {
    id: 'turn-flow',
    target: 'prompt',
    title: 'One Turn, Two Drivers',
    body: 'A normal round starts with the Lead Driver, then alternates to the Following Driver. On your turn you choose a card or skip, resolve any card choices, move using MP, then clean up and draw back up to 4 cards. After both drivers act, the game checks who is leading and starts the next round.',
    trigger: ({ gameState, canControlActivePlayer }) => canControlActivePlayer && gameState.phase === 'PLAY_CARD' && gameState.tutorial?.seen?.['race-start']
  },
  {
    id: 'hand',
    target: 'hand',
    title: 'Driving Cards',
    body: 'These are your driving cards: the moves your driver can commit to before the car moves. Gas cards build speed, Brake cards slow you down, Turn cards help with cornering and lane control, and Attack cards pressure the other racer.',
    trigger: ({ activeHandVisible, activeHandCanPlay }) => activeHandVisible && activeHandCanPlay
  },
  {
    id: 'card-requirements',
    target: 'hand',
    title: 'Card Requirements',
    body: 'Some cards have speed requirements, such as Min 60 or Max 80. That means the card only works when your current speed is inside that range. If a card is dimmed, your car is not in the right speed window yet.',
    trigger: ({ activeHandVisible, activeHandCanPlay, visibleHandPlayer, activePlayer, CARDS }) => activeHandVisible && activeHandCanPlay && visibleHandPlayer?.hand?.some(id => !CARDS[id]?.canPlay(activePlayer))
  },
  {
    id: 'skip-turn',
    target: 'hand',
    title: 'Skip/Refresh',
    body: 'Skip/Refresh means you choose not to play a driving card this turn and instead move into the discard step. Use it when your hand is awkward: discard cards you do not want, dig deeper into the deck, then draw back up to 4 cards at the end of the turn.',
    trigger: ({ gameState, activePlayer, canControlActivePlayer }) => canControlActivePlayer && gameState.phase === 'PLAY_CARD' && activePlayer?.id === 0 && (gameState.playerTurnsStarted?.[0] || 0) >= 2
  },
  {
    id: 'movement',
    target: 'movement',
    title: 'Move Points',
    body: 'Move Points, or MP, are the number of spaces your car can spend this movement step. The highlighted spaces are legal choices right now. Moving forward advances the race, while changing lanes can set up a cleaner corner line or an overtake.',
    trigger: ({ gameState, canControlActivePlayer, validMovesList }) => canControlActivePlayer && ['MOVE', 'NN_MOVE'].includes(gameState.phase) && validMovesList.length > 0
  },
  {
    id: 'speed-limit',
    target: 'speed-limit',
    title: 'Corner Speed Limits',
    body: 'A corner speed limit is the safe speed for that part of the road. Red asphalt and roadside signs mark speed checks. If your speed is above the limit when you pass one, the car understeers: it pushes wide and takes a penalty.',
    trigger: ({ gameState, canControlActivePlayer, activePlayer, TRACK }) => {
      if (!canControlActivePlayer || !['MOVE', 'NN_MOVE'].includes(gameState.phase)) return false;
      const lane = TRACK[activePlayer.lane] || [];
      return lane.slice(activePlayer.idx, activePlayer.idx + 5).some(space => space?.limit);
    }
  },
  {
    id: 'slipstream',
    target: 'prompt',
    title: 'Slipstream',
    body: 'Slipstream is the draft behind another car. When you are following in the same lane, you can use that pocket of air to gain speed. It is a catch-up tool, but extra speed can also make the next corner riskier.',
    trigger: ({ gameState, canControlActivePlayer }) => canControlActivePlayer && gameState.phase === 'SLIPSTREAM'
  },
  {
    id: 'neck-and-neck',
    target: 'prompt',
    title: 'Neck and Neck',
    body: 'Neck and Neck means both racers are side by side and fighting for the same stretch of road. In this state, both drivers commit cards face down, reveal together, then alternate movement one point at a time.',
    trigger: ({ gameState, canControlActivePlayer }) => canControlActivePlayer && ['NN_SELECT_CARD', 'NN_REVEAL', 'NN_MOVE'].includes(gameState.phase)
  },
  {
    id: 'nn-action-order',
    target: 'prompt',
    title: 'Simultaneous Reveal',
    body: 'During the reveal, both face-down cards are shown before movement begins. The first movement step is decided by the cars before card effects fully resolve: higher speed acts first, then the inside lane wins ties, and if it is still tied the previous order is used.',
    trigger: ({ gameState, nnRevealStage }) => gameState.phase === 'NN_REVEAL' && nnRevealStage === 'decision'
  },
  {
    id: 'overtake',
    target: 'active-car',
    title: 'Overtakes',
    body: 'An overtake happens when one car passes the other on the road. It is more than a flashy moment: it can change who counts as the Lead Driver, which usually changes who acts first on the next turn.',
    trigger: ({ gameState }) => Boolean(gameState.overtakeEvent)
  },
  {
    id: 'crash-understeer',
    target: 'speed-limit',
    title: 'Understeer and Crashes',
    body: 'Understeer means the car is going too fast to hold the corner, so it slides wide instead of following the line. If the mistake is severe, it can become a crash or spin-off. Brake early or use cornering cards to stay in control.',
    trigger: ({ gameState }) => Boolean(gameState.crashEvent)
  }
];

const TutorialOverlay = ({ step, onDismiss, onSkip, t = (_, fallback) => fallback }) => {
  if (!step) return null;
  const title = t(`tutorial.${step.id}.title`, step.title);
  const body = t(`tutorial.${step.id}.body`, step.body);
  return (
    <div className="fixed left-1/2 top-24 z-[150] w-[min(92vw,430px)] -translate-x-1/2 no-pan pointer-events-auto">
      <div className="border-2 border-yellow-300 bg-zinc-950/95 p-5 shadow-[0_0_40px_rgba(250,204,21,0.28)] backdrop-blur-md">
        <div className="text-[10px] font-black uppercase tracking-[0.35em] text-yellow-300">{t('ui.tutorial', 'Tutorial')}</div>
        <h3 className="mt-2 text-2xl font-black italic uppercase text-white">{title}</h3>
        <p className="mt-3 text-sm font-bold leading-relaxed text-zinc-300">{body}</p>
        <div className="mt-5 flex flex-wrap items-center justify-end gap-3">
          <button onClick={onSkip} className="border border-zinc-700 bg-zinc-900 px-4 py-2 text-xs font-black uppercase tracking-widest text-zinc-400 transition hover:border-white hover:text-white">{t('ui.tutorial_skip', 'Skip Tutorial')}</button>
          <button onClick={onDismiss} className="bg-yellow-400 px-5 py-2 text-sm font-black uppercase tracking-wider text-black transition hover:bg-yellow-300">{t('ui.got_it', 'Got It')}</button>
        </div>
      </div>
    </div>
  );
...
const loadPeerLibrary = () => new Promise((resolve, reject) => {
  if (typeof window === 'undefined') {
    reject(new Error('Peer networking is only available in a browser.'));
    return;
  }
  if (window.Peer) {
    resolve(window.Peer);
  ...
  const existing = document.querySelector(`script[src="${PEER_SCRIPT_URL}"]`);
  if (existing) {
    existing.addEventListener('load', () => resolve(window.Peer), { once: true });
    existing.addEventListener('error', () => reject(new Error('Peer networking failed to load.')), { once: true });
    return;
  }
  const script = document.createElement('script');
  script.src = PEER_SCRIPT_URL;
  script.async = true;
  script.onload = () => window.Peer ? resolve(window.Peer) : reject(new Error('Peer networking is unavailable.'));
  script.onerror = () => reject(new Error('Peer networking failed to load.'));
...
export default function ArcadeRacingGame() {
  ...
  const onlineClientIdRef = useRef(`${Date.now()}-${Math.random().toString(36).slice(2)}`);
  const onlineChannelRef = useRef(null);
  const onlinePeerRef = useRef(null);
  const onlinePeerConnectionRef = useRef(null);
  const onlineMessageHandlerRef = useRef(null);
  const onlineRoleRef = useRef(null);
  const onlineRoomKeyRef = useRef('');
  const applyingRemoteSelectionRef = useRef(false);
  const applyingRemoteGameStateRef = useRef(false);
  ...
  })), [cornerCardRows, translationTable, language]);
  const roadDebugLayout = useMemo(() => buildRoadDebugLayout(ROAD_CARDS), [ROAD_CARDS]);
  const racingCodexCards = useMemo(() => IMPLEMENTED_CARD_IDS.map(id => CARDS[id]).filter(Boolean), [CARDS]);
  ...
  useEffect(() => {
    let cancelled = false;
    const url = `localization.csv?updated=${Date.now()}`;

    fetch(url, { cache: 'no-store' })
      .then(response => {
        if (!response.ok) throw new Error(`Localization CSV request failed: ${response.status}`);
      ...
      .then(text => {
        ...
        if (!cancelled) {
          ...
          setLocalizationDataSource('csv');
        }
      })
      .catch(() => {
        if (!cancelled) setLocalizationDataSource('fallback');
      });
  ...
  useEffect(() => {
    onlineRoleRef.current = onlineSession.role;
    onlineRoomKeyRef.current = onlineSession.roomKey;
  }, [onlineSession.role, onlineSession.roomKey]);

  const postOnlineMessage = (message) => {
    const payload = {
      ...message,
      roomKey: message.roomKey || onlineRoomKeyRef.current,
      senderId: onlineClientIdRef.current,
      messageId: `${Date.now()}-${Math.random().toString(36).slice(2)}`
    };
    if (onlinePeerConnectionRef.current?.open) {
      onlinePeerConnectionRef.current.send(payload);
    }
    onlineChannelRef.current?.postMessage(payload);
    if (typeof window !== 'undefined' && window.localStorage && payload.roomKey) {
      window.localStorage.setItem(`arcade-racing-room-${payload.roomKey}-message`, JSON.stringify(payload));
    }
  };

  const attachPeerConnection = (connection) => {
    onlinePeerConnectionRef.current = connection;
    connection.on('open', () => {
      if (onlineRoleRef.current === 'host') {
        setOnlineSession(current => ({ ...current, guestConnected: true, message: t('online.guest_connected_waiting_ready', 'Guest connected. Waiting for ready signal.') }));
        postOnlineMessage({ type: 'host-ack', roomKey: onlineRoomKeyRef.current, senderRole: 'host' });
      } else {
        setOnlineSession(current => ({ ...current, guestConnected: true, message: t('online.connected_mark_ready', 'Connected to host. Mark ready when you are set.') }));
        postOnlineMessage({ type: 'guest-join', roomKey: onlineRoomKeyRef.current, senderRole: 'guest' });
      }
    });
    connection.on('data', message => onlineMessageHandlerRef.current?.(message));
    connection.on('close', () => {
      onlinePeerConnectionRef.current = null;
      setOnlineSession(current => ({ ...current, guestConnected: false, guestReady: false, message: t('online.connection_closed', 'Connection closed. Rejoin or host a new room.') }));
    });
    connection.on('error', () => {
      setOnlineSession(current => ({ ...current, message: t('online.peer_error', 'Peer connection error. Check the room key or retry.') }));
    });
  };

  const startPeerNetworking = async (role, roomKey) => {
    try {
      const Peer = await loadPeerLibrary();
      onlinePeerConnectionRef.current?.close?.();
      onlinePeerRef.current?.destroy?.();
      const peerId = role === 'host' ? `arcade-racing-${roomKey}` : undefined;
      const peer = new Peer(peerId);
      onlinePeerRef.current = peer;
      peer.on('connection', attachPeerConnection);
      peer.on('error', () => {
        setOnlineSession(current => ({ ...current, message: t('online.broker_unavailable', 'Online broker unavailable. Same-browser fallback is still active.') }));
      });
      if (role === 'guest') {
        peer.on('open', () => {
          attachPeerConnection(peer.connect(`arcade-racing-${roomKey}`, { reliable: true }));
        });
      }
    } catch {
      setOnlineSession(current => ({ ...current, message: t('online.networking_unavailable', 'Online networking could not load. Same-browser fallback is still active.') }));
    }
  };
    ...
    const handleOnlineMessage = (message) => {
      ...
      if (onlineRoleRef.current === 'host') {
        ...
        if (message.type === 'guest-ready') {
          setOnlineSession(current => ({ ...current, guestConnected: true, guestReady: true, message: t('online.guest_ready_start_setup', 'Guest is ready. You can start the race setup.') }));
        }
      }

      if (onlineRoleRef.current === 'guest') {
        if (message.type === 'host-ack') {
          setOnlineSession(current => ({ ...current, guestConnected: true, message: t('online.connected_mark_ready', 'Connected to host. Mark ready when you are set.') }));
        }
        if (message.type === 'start-select') {
          setGameMode('OnlinePvP');
      ...
      if (message.type === 'selection-state' && message.selection) {
        applyingRemoteSelectionRef.current = true;
        setGameMode('OnlinePvP');
        setSelectedCourseId(message.selection.selectedCourseId);
      ...
      if (message.type === 'game-state' && message.gameState) {
        applyingRemoteGameStateRef.current = true;
        setGameMode('OnlinePvP');
        setTrackSeed(message.trackSeed);
        setTrackCardCount(message.trackCardCount);
        setGameState(message.gameState);
        if (typeof message.gameState.tutorial?.enabled === 'boolean') setTutorialEnabled(message.gameState.tutorial.enabled);
        setActiveTutorialId(null);
        setAppState(message.appState || 'PLAYING');
        setTimeout(() => { applyingRemoteGameStateRef.current = false; }, 0);
      }
    };
    onlineMessageHandlerRef.current = handleOnlineMessage;

    let channel = null;
    if (window.BroadcastChannel) {
      channel = new BroadcastChannel(`arcade-racing-room-${onlineSession.roomKey}`);
      onlineChannelRef.current = channel;
      channel.onmessage = (event) => handleOnlineMessage(event.data || {});
    }

    const handleStorage = (event) => {
      if (event.key !== `arcade-racing-room-${onlineSession.roomKey}-message` || !event.newValue) return;
      try {
        handleOnlineMessage(JSON.parse(event.newValue));
      } catch {
        // Ignore malformed room traffic from another tab.
      }
    };
    window.addEventListener('storage', handleStorage);

    if (onlineSession.role === 'guest') {
      postOnlineMessage({ type: 'guest-join', roomKey: onlineSession.roomKey, senderRole: 'guest' });
    }

    return () => {
      window.removeEventListener('storage', handleStorage);
      channel?.close();
      if (onlineChannelRef.current === channel) onlineChannelRef.current = null;
      if (onlineMessageHandlerRef.current === handleOnlineMessage) onlineMessageHandlerRef.current = null;
    };
  }, [appState, onlineSession.role, onlineSession.roomKey]);

  useEffect(() => {
    if (gameMode !== 'OnlinePvP' || appState !== 'SELECT' || applyingRemoteSelectionRef.current) return;
    postOnlineMessage({
  ...
  useEffect(() => {
    ...
    if (applyingRemoteGameStateRef.current) {
      applyingRemoteGameStateRef.current = false;
      return;
    }
    postOnlineMessage({
      type: 'game-state',
  ...
  }, [gameMode, appState, gameState, onlineSession.roomKey, onlineSession.role, trackSeed, trackCardCount]);
  ...
  const openOnlineMatchmaking = () => {
    setGameMode('OnlinePvP');
    onlinePeerConnectionRef.current?.close?.();
    onlinePeerRef.current?.destroy?.();
    onlinePeerConnectionRef.current = null;
    onlinePeerRef.current = null;
    setOnlineSession(ONLINE_SESSION_INITIAL);
    setSelectedCourseId('akagi');
    setCourseDirection('downhill');
    setRandomMapSize('medium');
    setSelectedCars([null, null]);
  ...
  const createOnlineRoom = () => {
    ...
    setOnlineSession({
      ...ONLINE_SESSION_INITIAL,
      role: 'host',
      roomKey,
      message: t('online.share_room_key', 'Share this room key with your guest.')
    });
    startPeerNetworking('host', roomKey);
  };

  const joinOnlineRoom = () => {
    const roomKey = onlineSession.guestInput.trim().toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 8);
    if (!roomKey) {
      setOnlineSession(current => ({ ...current, message: t('online.enter_room_key', 'Enter a room key first.') }));
      return;
    }
    setOnlineSession(current => ({
      ...
      guestReady: false,
      guestConnected: false,
      message: t('online.looking_for_host', 'Looking for host...')
    }));
    startPeerNetworking('guest', roomKey);
  };

  const setGuestReady = () => {
    postOnlineMessage({ type: 'guest-ready', roomKey: onlineSession.roomKey, senderRole: 'guest' });
    setOnlineSession(current => ({ ...current, guestReady: true, message: t('online.ready_sent', 'Ready sent. Waiting for the host to start.') }));
  };

  const startOnlineSelection = () => {
    if (!onlineSession.guestConnected || !onlineSession.guestReady) return;
    postOnlineMessage({ type: 'start-select', roomKey: onlineSession.roomKey, senderRole: 'host' });
    setGameMode('OnlinePvP');
    setSelectedCars([null, null]);
    setActiveCarPlayer(0);
    setAppState('SELECT');
  ...
  const openRoadDebugMap = () => {
    ...
    setZoom(winSize.w < 640 ? 0.3 : 0.55);
    setAppState('ROAD_DEBUG');
  };
  ...
  useEffect(() => {
    if (gameState?.drawAnimation) {
      const timer = setTimeout(() => setGameState(prev => prev ? { ...prev, drawAnimation: null } : prev), 900);
      return () => clearTimeout(timer);
    }
  ...
  useEffect(() => {
    ...
    const bannerPhases = ['PLAY_CARD', 'NN_SELECT_CARD', 'NN_MOVE'];
    if (!bannerPhases.includes(gameState.phase)) return;

    const key = `${gameState.phase}-${activePlayerId}-${gameState.activePlayerIdx}-${gameState.nnActiveTurnIdx ?? 'normal'}`;
    const label = gameState.phase === 'NN_MOVE' ? t('ui.movement_step', 'Movement Step') : gameState.phase === 'NN_SELECT_CARD' ? t('ui.lock_in_card', 'Lock In Card') : t('ui.turn_start', 'Turn Start');
    setTurnBanner({ key, playerId: activePlayerId, playerName: activePlayer.name, label });

    const timer = setTimeout(() => {
      setTurnBanner(current => current?.key === key ? null : current);
    }, 1500);
    return () => clearTimeout(timer);
  }, [gameState?.phase, gameState?.activePlayerIdx, gameState?.nnActiveTurnIdx, activePlayerId]);
  ...
  const activeTutorialStep = TUTORIAL_STEPS.find(step => step.id === activeTutorialId) || null;
  const activeTutorialTarget = activeTutorialStep?.target || null;

  const markTutorialSeen = (id) => {
    if (!id) return;
    setGameState(prev => {
      ...
      return {
        ...
        tutorial: {
          ...prev.tutorial,
          seen: { ...(prev.tutorial.seen || {}), [id]: true }
        }
      };
    });
  };
  ...
  const dismissTutorial = () => {
    markTutorialSeen(activeTutorialId);
    setActiveTutorialId(null);
  };

  const skipTutorial = () => {
    setGameState(prev => prev?.tutorial ? { ...prev, tutorial: { ...prev.tutorial, enabled: false } } : prev);
    setActiveTutorialId(null);
  };

  useEffect(() => {
    if (appState !== 'PLAYING' || !gameState?.tutorial?.enabled || gameState.winner || activeTutorialId) return;
    const tutorialActivePlayerId = gameState.phase === 'NN_MOVE'
      ? gameState.nnActionOrder?.[gameState.nnActiveTurnIdx]
      : gameState.phase === 'NN_CARD_CYCLE'
        ? gameState.cycleQueue?.[gameState.cycleQueueIndex]
        : gameState.turnOrder?.[gameState.activePlayerIdx];
    const tutorialActivePlayer = gameState.players?.[tutorialActivePlayerId];
    if (!tutorialActivePlayer) return;
    const tutorialIsOnlineGame = gameMode === 'OnlinePvP';
    const tutorialLocalPlayerId = tutorialIsOnlineGame ? (onlineSession.role === 'guest' ? 1 : 0) : tutorialActivePlayerId;
    const tutorialIsLocalPlayersTurn = !tutorialIsOnlineGame || tutorialLocalPlayerId === tutorialActivePlayerId;
    const tutorialIsAITurn = gameMode === 'PvE' && tutorialActivePlayerId === 1;
    const tutorialCanControlActivePlayer = !tutorialIsAITurn && tutorialIsLocalPlayersTurn;
    const tutorialShouldShowLocalHand = !tutorialIsAITurn && (!tutorialIsOnlineGame || tutorialIsLocalPlayersTurn);
    const tutorialActiveHandVisible = tutorialShouldShowLocalHand && ['PLAY_CARD', 'NN_SELECT_CARD', 'MOVE', 'NN_MOVE'].includes(gameState.phase);
    const tutorialActiveHandCanPlay = tutorialIsLocalPlayersTurn && ['PLAY_CARD', 'NN_SELECT_CARD'].includes(gameState.phase);
    const tutorialVisibleHandPlayer = tutorialIsOnlineGame ? gameState.players?.[tutorialLocalPlayerId] : tutorialActivePlayer;
    const tutorialValidMovesList = ['MOVE', 'NN_MOVE'].includes(gameState.phase) && tutorialCanControlActivePlayer
      && ((gameState.phase === 'MOVE' && gameState.mpLeft > 0) || (gameState.phase === 'NN_MOVE' && gameState.nnMpLeft?.[tutorialActivePlayerId] > 0))
      ? getValidMoves(tutorialActivePlayer)
      : [];

    const context = {
      gameState,
      activePlayer: tutorialActivePlayer,
      canControlActivePlayer: tutorialCanControlActivePlayer,
      activeHandVisible: tutorialActiveHandVisible,
      activeHandCanPlay: tutorialActiveHandCanPlay,
      visibleHandPlayer: tutorialVisibleHandPlayer,
      validMovesList: tutorialValidMovesList,
      nnRevealStage,
      CARDS,
      TRACK
    };
    const nextStep = TUTORIAL_STEPS.find(step => !gameState.tutorial.seen?.[step.id] && step.trigger(context));
    if (nextStep) setActiveTutorialId(nextStep.id);
  }, [
    appState,
    gameState?.phase,
    gameState?.activePlayerIdx,
    gameState?.nnActiveTurnIdx,
    gameState?.mpLeft,
    gameState?.nnMpLeft?.[0],
    gameState?.nnMpLeft?.[1],
    gameState?.overtakeEvent,
    gameState?.crashEvent,
    gameState?.tutorial?.enabled,
    gameState?.tutorial?.seen,
    activeTutorialId,
    nnRevealStage,
    gameMode,
    onlineSession.role,
  ...
  const playCard = (cardId, handIndex) => {
    ...
    if (gameState.phase === 'NN_SELECT_CARD') {
      addLog(t('log.locked_card', '{player} locked in a card.', { player: activePlayer.name }));
    ...
    addLog(t('log.played_card', '{player} played {card}.', { player: activePlayer.name, card: card.name }));
    ...
    if (canPlayExtraCard) {
      ...
      nextPhase = 'PLAY_CARD';
      mp = 0;
      addLog(t('log.may_play_another', '{player} may play another card.', { player: p.name }));
    }
    setDiscardSelection([]);
    setGameState(prev => ({
      ...prev,
      ...
      mangaCutin: { card: cardId, player: p.name, color: p.id === 0 ? 'text-red-500' : 'text-blue-500', type: card.rulesType || card.type }
    }));
    setHoveredCardIdx(null);
  };
  ...
  const skipToDiscard = () => {
    if (isOnlineGame && !isLocalPlayersTurn) return;
    addLog(t('log.skips_card', '{player} skips card play.', { player: activePlayer.name }));
    setDiscardSelection([]);
  ...
  const confirmDiscard = (overrideSelection = null) => {
    ...
    if (discardedIds.length > 0) addLog(t('log.discarded_cards', '{player} discarded {count} cards.', { player: p.name, count: discardedIds.length }));
  ...
  const confirmCardCycle = (overrideSelection = null) => {
    ...
    if (discardedIds.length > 0) addLog(t('log.cycles_cards', '{player} cycles {count} card.', { player: player.name, count: discardedIds.length }));
  ...
  const acceptSlipstream = (accept) => {
    ...
    if (accept) {
      ...
      addLog(t('log.takes_slipstream', '{player} takes the Slipstream! (+10 km/h)', { player: p.name }));
    } else {
      addLog(t('log.declined_slipstream', '{player} declined the Slipstream.', { player: p.name }));
    }
  ...
  const attemptMove = (targetLane, targetIdx) => {
    ...
    if (op.modifiers.noRoomLane !== undefined && targetLane === op.modifiers.noRoomLane && !(targetLane === op.lane && targetIdx === op.idx)) {
      addLog(t('log.no_room', 'NO ROOM: {opponent} blocks {player} from entering that lane.', { player: p.name, opponent: op.name }));
      return;
    }

    if (targetLane === op.lane && targetIdx === op.idx) {
      addLog(t('log.blocked_by', '{player} is blocked by {opponent}!', { player: p.name, opponent: op.name }));
      if (p.modifiers.chromeBumper && p.hand.length > 0) {
        ...
        addLog(t('log.chrome_bumper', 'CHROME BUMPER: {opponent} loses 40 km/h.', { opponent: op.name }));
      }
      if (p.modifiers.doorSlam) {
        op = { ...op, speed: clampSpeed(op.speed - 20), modifiers: { ...op.modifiers, skipRemainingMove: true } };
        addLog(t('log.door_slam', 'DOOR SLAM: {opponent} loses 20 km/h and remaining move points.', { opponent: op.name }));
      }
      if (p.modifiers.shoveAside) {
        ...
        if (!blockedShove) {
          ...
          addLog(t('log.shove_aside', 'SHOVE ASIDE: {player} forces {opponent} out of the lane.', { player: p.name, opponent: op.name }));
          newPlayers[activePlayerId] = p;
        ...
        addLog(t('log.shove_aside_no_space', 'SHOVE ASIDE: no space. Both cars spin off and lose remaining move points.'));
        newPlayers[activePlayerId] = p;
        newPlayers[1 - activePlayerId] = op;
      ...
      if (p.modifiers.blindAttack) {
        ...
        if (!targetOccupied) {
          ...
          addLog(t('log.blind_attack', 'BLIND ATTACK: {player} switches lanes and keeps momentum.', { player: p.name }));
          newPlayers[activePlayerId] = p;
      ...
      if (p.speed > op.speed) {
        p.speed = op.speed;
        addLog(t('log.matches_speed', '{player} matches speed to {speed}.', { player: p.name, speed: op.speed }));
      }
      newPlayers[activePlayerId] = p;
      newPlayers[1 - activePlayerId] = op;
      if (isNN) advanceNNTurn(newPlayers, activePlayerId, mpAvail - 1, triggeredCrash || gameState.crashEvent);
    ...
    if (movedSideways && p.modifiers.balancedChassis && p.hand.length > 0) {
      ...
      currentMp = mpAvail;
      addLog(t('log.balanced_chassis', 'BALANCED CHASSIS: {player} discards 1 card for a free side step.', { player: p.name }));
    }
    ...
    while (understeering) {
      ...
      if (effLimit !== null) {
        ...
        if (p.modifiers.trailBraking && p.speed > effLimit && p.speed - 10 >= 0) {
          ...
          addLog(t('log.trail_braking', 'TRAIL BRAKING: {player} sheds 10 km/h before the speed check.', { player: p.name }));
        }
      }
      ...
      if (effLimit !== null && p.speed > effLimit && !p.modifiers.ignoreSpeedLimits) {
        if (p.modifiers.tractionControl) {
          ...
          addLog(t('log.traction_control', 'TRACTION CONTROL: {player} loses 20 km/h and cancels understeer.', { player: p.name }));
          understeering = false;
          continue;
        }
        if (p.modifiers.countersteerCancel && p.hand.length > 0) {
          ...
          addLog(t('log.full_countersteer', 'FULL COUNTERSTEER: {player} discards 1 card and cancels understeer.', { player: p.name }));
          understeering = false;
          continue;
        }
        addLog(t('log.understeer', 'UNDERSTEER: {player} speed {speed} exceeds limit {limit}.', { player: p.name, speed: p.speed, limit: effLimit }));
        ...
        if (space.isOuter) {
          ...
          if (space.guardrail) {
            ...
            addLog(t('log.crash_guardrail', 'CRASH: {player} hits the guardrail. (-1 Gear, remaining move points lost)', { player: p.name }));
            p.speed = getPenaltySpeed(pGear - 1);
          } else {
            triggeredCrash = { name: p.name, type: 'SPIN OFF', penalty: '-2 Gears' };
            addLog(t('log.spin_off', 'SPIN OFF: {player} leaves the road. (-2 Gears, remaining move points lost)', { player: p.name }));
          ...
          // Push outward
          const { lane: outLane, idx: outIdx } = getUndersteerTarget(p.lane, p.idx, TRACK);
          p.lane = outLane;
          p.idx = outIdx;
          ...
          if (p.lane === op.lane && p.idx === op.idx) {
            ...
            addLog(t('log.collision', 'COLLISION: {player} side-swipes {opponent}.', { player: p.name, opponent: op.name }));
    ...
    if (p.modifiers.progressiveAcceleration) {
      ...
      addLog(t('log.progressive_acceleration', 'PROGRESSIVE ACCELERATION: {player} gains 10 km/h.', { player: p.name }));
    }
    ...
    setGameState(prev => {
      ...
      [0, 1].forEach(pid => {
         ...
         if (p.modifiers.rocketSpeedDelta) {
           ...
           newLogs.unshift(t('log.rocket_start', 'ROCKET START: {player} gains {amount} km/h.', { player: p.name, amount: p.modifiers.rocketSpeedDelta }));
         }
         if (p.modifiers.exitDriftPending && p.modifiers.passedSpeedCheck) {
           p.speed = clampSpeed(p.speed + 30);
           newLogs.unshift(t('log.exit_drift', 'EXIT DRIFT: {player} gains 30 km/h.', { player: p.name }));
         }
         if (p.modifiers.earlyPowerPending && p.modifiers.passedSpeedCheck && !p.modifiers.speedCheckFailed) {
           p.speed = clampSpeed(p.speed + 40);
           newLogs.unshift(t('log.early_power', 'EARLY POWER: {player} gains 40 km/h.', { player: p.name }));
         }
         if (p.modifiers.burnRubberPending && p.distance > newPlayers[1 - pid].distance) {
           ...
           p.hand = [];
           newLogs.unshift(t('log.burn_rubber', 'BURN RUBBER: {player} gains 60 km/h and discards all cards.', { player: p.name }));
         }
         ...
         p.modifiers = {};
         const handCountBeforeDraw = p.hand.length;
         const drawn = drawCards(p.deck, p.discard, p.hand, 4);
         p.deck = drawn.deck; p.discard = drawn.discard; p.hand = drawn.hand;
         p.cardsDrawnLastCleanup = Math.max(0, p.hand.length - handCountBeforeDraw);
         p.lastPlayed = null;
         newPlayers[pid] = p;
      });
      newPlayers = newPlayers.map(({ cardsDrawnLastCleanup, ...player }) => player);
      ...
      if (newTurnOrder[0] !== prev.turnOrder[0]) {
        ...
        newLogs.unshift(t('log.overtake_lead', 'OVERTAKE: {player} takes the lead!', { player: newPlayers[newTurnOrder[0]].name }));
      }

      newLogs.unshift(t('log.nn_complete', '-- N&N Complete. {player} is Lead. --', { player: newPlayers[newTurnOrder[0]].name }));

      const isNextNN = p1.lane !== p2.lane && TRACK[p1.lane][p1.idx].adj.includes(p2.idx);
      const nextStarted = !isNextNN ? {
        ...(prev.playerTurnsStarted || {}),
        [newTurnOrder[0]]: ((prev.playerTurnsStarted || {})[newTurnOrder[0]] || 0) + 1
      } : (prev.playerTurnsStarted || {});

      return {
         ...prev, players: newPlayers, turnOrder: newTurnOrder,
         activePlayerIdx: 0,
         playerTurnsStarted: nextStarted,
  ...
  const endTurn = () => {
    setGameState(prev => {
      ...
      p.hand = drawn.hand;
      const cardsDrawn = Math.max(0, p.hand.length - handCountBeforeDraw);
      ...
      if (isNNNow) {
        ...
        newLogs.unshift(t('log.nn_locked', 'NECK AND NECK: Both cards are locked in.'));
        return {
          ...prev,
          players: newPlayers,
          activePlayerIdx: 0,
          ...
          nnActiveTurnIdx: null,
          nnMpLeft: null,
          drawAnimation: cardsDrawn > 0 && (isOnlineGame ? p.id === localPlayerId : gameMode !== 'PvE' || p.id === 0) ? { key: Date.now(), playerId: p.id, count: cardsDrawn } : null,
          isNNRound: true
        };
      }
      ...
      if (nextIdx >= 2) {
        ...
        newLogs.unshift(t('log.round_end', '-- Round End. {player} is Lead. --', { player: newPlayers[newTurnOrder[0]].name }));

        isNN = p1.lane !== p2.lane && TRACK[p1.lane][p1.idx].adj.includes(p2.idx);
        if (isNN) {
           nextPhase = 'NN_SELECT_CARD';
           newLogs.unshift(t('log.nn_engaged', 'NECK AND NECK ENGAGED!'));
        }
      }

      const nextActiveId = newTurnOrder[nextIdx];
      const nextStarted = nextPhase === 'PLAY_CARD' ? {
        ...(prev.playerTurnsStarted || {}),
        [nextActiveId]: ((prev.playerTurnsStarted || {})[nextActiveId] || 0) + 1
      } : (prev.playerTurnsStarted || {});
      ...
      return {
        ...
        nnSelections: isNN ? {0:null, 1:null} : null,
        drawAnimation: cardsDrawn > 0 && (isOnlineGame ? p.id === localPlayerId : gameMode !== 'PvE' || p.id === 0) ? { key: Date.now(), playerId: p.id, count: cardsDrawn } : null,
        isNNRound: isNN
      };
    });
  ...
  const Speedometer = ({ player, position, projectedSpeed, isTurn, phase, mpLeft, isLeadStatus, isTutorialTarget = false }) => {
    const gearInfo = getGear(player.speed);
    const speedToNeedleAngle = (speed) => {
      const points = [
        [0, 214],
        [20, 248],
        [50, 306.5],
      ...
      for (let i = 1; i < points.length; i++) {
        const [prevSpeed, prevAngle] = points[i - 1];
        const [nextSpeed, nextAngle] = points[i];
        if (clamped <= nextSpeed) {
          const t = (clamped - prevSpeed) / (nextSpeed - prevSpeed);
          return prevAngle + (nextAngle - prevAngle) * t;
        }
      }
      return points[points.length - 1][1];
    };
    const angle = speedToNeedleAngle(player.speed);
    const ghostAngle = projectedSpeed !== null ? speedToNeedleAngle(projectedSpeed) : null;
    ...
    return (
      <div className={`fixed ${meterPositionClass} z-40 flex w-[340px] flex-col bg-zinc-950/75 backdrop-blur-md p-3 rounded-2xl border border-white/15 shadow-2xl no-pan pointer-events-none ${isTutorialTarget ? 'tutorial-highlight' : ''}`}>

        {/* Lead / Following Badge */}
        <div className={`absolute -top-3 right-4 px-3 py-1 rounded-full text-[12px] font-black uppercase tracking-widest shadow-lg border ${isLeadStatus ? 'bg-yellow-400 text-black border-yellow-500 whitespace-nowrap' : 'bg-zinc-800 text-zinc-400 border-zinc-600 whitespace-nowrap'}`}>
          {isLeadStatus ? t('ui.leading', 'LEADING') : t('ui.following', 'FOLLOWING')}
        </div>

        <div className="mb-1 flex items-end justify-between gap-3 pr-20">
          <h3 className={`text-lg font-black uppercase tracking-wider leading-none ${isP1 ? 'text-red-500' : 'text-blue-500'}`}>{player.name}</h3>
        </div>

        <div className="relative aspect-square w-full overflow-visible">
          <img
            src={SPEED_METER_CARD_IMAGE}
            alt=""
            className="absolute inset-0 h-full w-full object-contain select-none drop-shadow-[0_0_14px_rgba(255,255,255,0.12)]"
            draggable="false"
          />

          <div className="absolute left-1/2 top-[40%] -translate-x-1/2 -translate-y-1/2 text-center">
            <div className="text-sm font-black text-zinc-300/90 drop-shadow-[0_2px_8px_rgba(0,0,0,0.95)]">{t('ui.speed', 'Speed')}</div>
            <div className="font-mono text-4xl font-black leading-none text-white drop-shadow-[0_4px_12px_rgba(0,0,0,0.95)]">{player.speed}</div>
          </div>

          <div className="absolute right-[8%] bottom-[8%] flex items-baseline gap-1 text-right drop-shadow-[0_3px_10px_rgba(0,0,0,0.95)]">
            <span className="text-lg font-black text-zinc-300">{t('ui.gear', 'Gear')}</span>
          ...
          {ghostAngle !== null && (
            <div
              className="absolute left-1/2 bottom-[47%] h-[38%] w-1 -translate-x-1/2 rounded-full bg-white/35 shadow-[0_0_10px_rgba(255,255,255,0.45)]"
              style={{ transform: `translateX(-50%) rotate(${ghostAngle}deg)`, transformOrigin: '50% 100%', transition: 'transform 0.3s cubic-bezier(0.34, 1.56, 0.64, 1)' }}
            />
          )}

          {/* Actual Needle */}
          <div
            className="absolute left-1/2 bottom-[47%] h-[38%] w-1.5 -translate-x-1/2 rounded-full bg-amber-300 shadow-[0_0_12px_rgba(251,191,36,0.9)]"
            style={{ transform: `translateX(-50%) rotate(${angle}deg)`, transformOrigin: '50% 100%', transition: 'transform 0.4s cubic-bezier(0.34, 1.56, 0.64, 1)' }}
          />
  ...
  const PileWidget = ({ type, count, topCard, onClick }) => {
    ...
    const title = isDiscard ? t('ui.discard', 'Discard') : t('ui.deck', 'Deck');
    return (
      <button
        type="button"
        onClick={onClick}
        disabled={!isDiscard || count === 0}
        className={`relative h-56 w-40 rounded-xl border-2 p-3 text-left shadow-2xl transition no-pan ${isDiscard ? 'border-orange-500/70 bg-zinc-900 hover:border-orange-300 hover:-translate-y-1 cursor-pointer' : 'border-zinc-600 bg-zinc-950 cursor-not-allowed'}`}
      >
          ...
          {isDiscard && topCard ? (
            topCard.imageSrc ? (
              ...
              <div className="h-full bg-zinc-800 p-2">
                <div className={`text-[10px] font-black uppercase ${(topCard.rulesType || topCard.type) === 'Gas' ? 'text-yellow-400' : (topCard.rulesType || topCard.type) === 'Brake' ? 'text-red-400' : 'text-blue-400'}`}>{topCard.type}</div>
  ...
  const DrawAnimation = ({ event }) => {
    if (!event) return null;
    const cardCount = Math.min(event.count || 1, 4);
    return (
      <div key={event.key} className="fixed bottom-24 right-24 z-[95] pointer-events-none no-pan">
  ...
  if (appState === 'TITLE') {
    ...
    const nextLanguage = LANGUAGES[(LANGUAGES.findIndex(item => item.code === language) + 1) % LANGUAGES.length] || LANGUAGES[0];
    const cardsSourceLabel = cardDataSource === 'csv' ? t('ui.live', 'LIVE') : t('ui.fallback', 'FALLBACK');
    const cornersSourceLabel = cornerDataSource === 'csv' ? t('ui.live', 'LIVE') : t('ui.fallback', 'FALLBACK');
    return (
      <div className="relative flex h-screen w-full overflow-hidden text-white flex-col items-center justify-center bg-[#050505]">
        <button
          ...
          className="fixed right-5 top-5 z-30 flex items-center gap-3 rounded-full border-2 border-yellow-400/70 bg-zinc-950/85 px-4 py-2 text-xs font-black uppercase tracking-[0.18em] text-yellow-100 shadow-[0_0_22px_rgba(250,204,21,0.2)] backdrop-blur-md transition hover:border-yellow-300 hover:bg-yellow-400 hover:text-black"
          aria-label={`${t('language.toggle', 'Language')}: ${t(currentLanguage.labelKey, currentLanguage.shortLabel)}`}
          title={`${t('language.toggle', 'Language')}: ${t(currentLanguage.labelKey, currentLanguage.shortLabel)}`}
        >
          <span>{currentLanguage.shortLabel}</span>
          <span className="text-[10px] opacity-70">{nextLanguage.shortLabel}</span>
        </button>
        ...
        />
        <p className="text-zinc-400 font-mono tracking-widest mb-12 uppercase">{t('ui.subtitle', 'TACTICAL DRIFT CARD BATTLES')}</p>
        <div className="flex flex-wrap justify-center gap-4">
          <button onClick={() => beginSetup('PvE')} className="group relative px-8 py-4 bg-zinc-900 border-2 border-zinc-700 rounded-xl hover:border-red-500 hover:bg-zinc-800 transition-all shadow-xl overflow-hidden">
            ...
            <div className="relative flex items-center gap-3 text-xl font-bold"><Bot className="text-zinc-400 group-hover:text-red-400" /><span>{t('ui.vs_ai_racers', 'VS AI Racers')}</span></div>
          </button>
          <button onClick={() => beginSetup('PvP')} className="group relative px-8 py-4 bg-zinc-900 border-2 border-zinc-700 rounded-xl hover:border-blue-500 hover:bg-zinc-800 transition-all shadow-xl overflow-hidden">
            ...
            <div className="relative flex items-center gap-3 text-xl font-bold"><Users className="text-zinc-400 group-hover:text-blue-400" /><span>{t('ui.local_2_player', 'Local 2 Player')}</span></div>
          </button>
          <button onClick={openOnlineMatchmaking} className="group relative px-8 py-4 bg-zinc-900 border-2 border-zinc-700 rounded-xl hover:border-emerald-400 hover:bg-zinc-800 transition-all shadow-xl overflow-hidden">
        ...
        <div className={`fixed bottom-6 px-4 py-2 rounded-full border text-xs font-bold tracking-wide ${cardDataSource === 'csv' && cornerDataSource === 'csv' ? 'bg-emerald-950/80 border-emerald-700 text-emerald-300' : cardDataSource === 'loading' || cornerDataSource === 'loading' ? 'bg-zinc-900/80 border-zinc-700 text-zinc-400' : 'bg-amber-950/80 border-amber-700 text-amber-300'}`}>
          {cardDataSource === 'loading' || cornerDataSource === 'loading'
            ? t('ui.loading_csv_data', 'LOADING CSV DATA...')
            : `${t('ui.cards_status', 'DRIVING CARDS: {cards} / CORNER CARDS: {corners}', { cards: cardsSourceLabel, corners: cornersSourceLabel })}${cardDataSource !== 'csv' || cornerDataSource !== 'csv' || localizationDataSource !== 'csv' ? ` / ${t('ui.run_live_csv', 'RUN start_arcade_racing.cmd FOR LIVE CSV')}` : ''}`}
        </div>
      </div>
    );
  }
  ...
  if (appState === 'MATCHMAKING') {
    const cleanGuestInput = onlineSession.guestInput.trim().toUpperCase().replace(/[^A-Z0-9]/g, '');
    const canHostStart = onlineSession.role === 'host' && onlineSession.guestConnected && onlineSession.guestReady;
      ...
      <div className="h-screen w-full overflow-y-auto bg-[#060808] text-white">
        <div className="flex min-h-screen flex-col bg-[radial-gradient(circle_at_48%_18%,rgba(16,185,129,0.2),transparent_36%),linear-gradient(125deg,rgba(127,29,29,0.18),transparent_34%,rgba(30,64,175,0.18))] px-4 py-5 sm:px-8">
          <header className="mx-auto flex w-full max-w-6xl items-center justify-between border-b border-white/15 pb-4">
            ...
            <div className="hidden rounded border border-emerald-400/30 bg-emerald-950/40 px-3 py-2 text-xs font-black uppercase text-emerald-200 sm:block">{onlineSession.role ? t(`ui.${onlineSession.role}`, onlineSession.role) : t('ui.choose_role', 'Choose Role')}</div>
          </header>

          <main className="mx-auto grid w-full max-w-6xl flex-1 items-stretch gap-4 py-6 lg:grid-cols-2">
            <section className={`border bg-black/68 p-5 shadow-2xl transition ${onlineSession.role === 'host' ? 'border-emerald-300 shadow-[0_0_28px_rgba(16,185,129,0.18)]' : 'border-white/15'}`}>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="text-[10px] font-black uppercase tracking-[0.32em] text-emerald-300">{t('ui.host', 'Host')}</div>
                  <h2 className="mt-2 text-3xl font-black italic uppercase">{t('ui.create_room', 'Create Room')}</h2>
                </div>
                <Radio className="text-emerald-300" size={34}/>
              </div>
              <p className="mt-4 text-sm font-bold leading-relaxed text-zinc-400">{t('ui.host_room_help', 'Generate a room key, wait for the guest to connect, then start once their ready light is on.')}</p>
              <button onClick={createOnlineRoom} className="mt-6 w-full border-2 border-emerald-300 bg-emerald-400 px-5 py-3 text-base font-black uppercase text-black transition hover:bg-emerald-300">{t('ui.host_a_room', 'Host A Room')}</button>

              <div className="mt-6 border border-white/10 bg-zinc-950/80 p-4">
                <div className="text-[10px] font-black uppercase tracking-[0.28em] text-zinc-500">{t('ui.room_key', 'Room Key')}</div>
                <div className="mt-3 flex min-h-20 items-center justify-center border border-dashed border-emerald-400/40 bg-black font-mono text-5xl font-black tracking-[0.22em] text-emerald-200">{onlineSession.role === 'host' ? onlineSession.roomKey : '------'}</div>
              </div>

              <div className="mt-4 grid grid-cols-2 gap-3">
                <div className={`border px-4 py-3 ${onlineSession.guestConnected ? 'border-emerald-400 bg-emerald-400/10 text-emerald-200' : 'border-white/15 bg-black/45 text-zinc-500'}`}>
                  <div className="text-[10px] font-black uppercase tracking-widest">{t('ui.guest', 'Guest')}</div>
                  <div className="mt-1 text-sm font-black uppercase">{onlineSession.guestConnected ? t('ui.connected', 'Connected') : t('ui.waiting', 'Waiting')}</div>
                </div>
                <div className={`border px-4 py-3 ${onlineSession.guestReady ? 'border-yellow-300 bg-yellow-300/10 text-yellow-200' : 'border-white/15 bg-black/45 text-zinc-500'}`}>
                  <div className="text-[10px] font-black uppercase tracking-widest">{t('ui.ready', 'Ready')}</div>
                  <div className="mt-1 text-sm font-black uppercase">{onlineSession.guestReady ? t('ui.locked_in', 'Locked In') : t('ui.not_yet', 'Not Yet')}</div>
                </div>
              </div>

              <button onClick={startOnlineSelection} disabled={!canHostStart} className={`mt-6 flex w-full items-center justify-center gap-3 border-2 px-5 py-4 text-lg font-black italic uppercase transition ${canHostStart ? 'border-yellow-300 bg-yellow-400 text-black hover:bg-yellow-300' : 'cursor-not-allowed border-white/10 bg-zinc-900 text-zinc-600'}`}>{t('ui.start_setup', 'Start Setup')} <Play size={20}/></button>
            </section>

            <section className={`border bg-black/68 p-5 shadow-2xl transition ${onlineSession.role === 'guest' ? 'border-blue-300 shadow-[0_0_28px_rgba(59,130,246,0.18)]' : 'border-white/15'}`}>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="text-[10px] font-black uppercase tracking-[0.32em] text-blue-300">{t('ui.guest', 'Guest')}</div>
                  <h2 className="mt-2 text-3xl font-black italic uppercase">{t('ui.join_room', 'Join Room')}</h2>
                </div>
                <Users className="text-blue-300" size={34}/>
              </div>
              <p className="mt-4 text-sm font-bold leading-relaxed text-zinc-400">{t('ui.join_room_help', 'Enter the host room key. Once connected, mark ready and wait for the host to launch setup.')}</p>
              <div className="mt-6">
                <label className="text-[10px] font-black uppercase tracking-[0.28em] text-zinc-500">{t('ui.room_key', 'Room Key')}</label>
                <input
                  value={onlineSession.guestInput}
                  onChange={event => setOnlineSession(current => ({ ...current, guestInput: event.target.value.toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 8) }))}
                  className="mt-3 w-full border-2 border-white/15 bg-zinc-950 px-4 py-4 text-center font-mono text-3xl font-black uppercase tracking-[0.18em] text-white outline-none transition focus:border-blue-300"
                  placeholder="ABC123"
                  maxLength={8}
                />
              </div>
              <button onClick={joinOnlineRoom} disabled={!cleanGuestInput} className={`mt-4 w-full border-2 px-5 py-3 text-base font-black uppercase transition ${cleanGuestInput ? 'border-blue-300 bg-blue-400 text-black hover:bg-blue-300' : 'cursor-not-allowed border-white/10 bg-zinc-900 text-zinc-600'}`}>{t('ui.join_as_guest', 'Join As Guest')}</button>

              <div className="mt-6 grid grid-cols-2 gap-3">
                <div className={`border px-4 py-3 ${onlineSession.role === 'guest' && onlineSession.guestConnected ? 'border-blue-300 bg-blue-400/10 text-blue-100' : 'border-white/15 bg-black/45 text-zinc-500'}`}>
                  <div className="text-[10px] font-black uppercase tracking-widest">{t('ui.host', 'Host')}</div>
                  <div className="mt-1 text-sm font-black uppercase">{onlineSession.role === 'guest' && onlineSession.guestConnected ? t('ui.found', 'Found') : t('ui.searching', 'Searching')}</div>
                </div>
                <div className={`border px-4 py-3 ${onlineSession.role === 'guest' && onlineSession.guestReady ? 'border-yellow-300 bg-yellow-300/10 text-yellow-200' : 'border-white/15 bg-black/45 text-zinc-500'}`}>
                  <div className="text-[10px] font-black uppercase tracking-widest">{t('ui.ready', 'Ready')}</div>
                  <div className="mt-1 text-sm font-black uppercase">{onlineSession.role === 'guest' && onlineSession.guestReady ? t('ui.sent', 'Sent') : t('ui.open', 'Open')}</div>
                </div>
              </div>

              <button onClick={setGuestReady} disabled={onlineSession.role !== 'guest' || !onlineSession.guestConnected || onlineSession.guestReady} className={`mt-6 w-full border-2 px-5 py-4 text-lg font-black italic uppercase transition ${onlineSession.role === 'guest' && onlineSession.guestConnected && !onlineSession.guestReady ? 'border-yellow-300 bg-yellow-400 text-black hover:bg-yellow-300' : 'cursor-not-allowed border-white/10 bg-zinc-900 text-zinc-600'}`}>{t('ui.ready', 'Ready')}</button>
            </section>
          </main>

          <div className="mx-auto mb-4 w-full max-w-6xl border border-white/15 bg-black/70 px-4 py-3 text-center text-xs font-black uppercase tracking-[0.18em] text-zinc-300">
            {onlineSession.message || 'Choose host or guest to begin matchmaking.'}
          </div>
        </div>
      </div>
    );
  ...
  if (appState === 'ROAD_DEBUG') {
    const debugBounds = roadDebugLayout.bounds;
    const debugBackdropPadding = TERRAIN_BACKDROP_PADDING;
    const debugBackdropBounds = {
      x: debugBounds.minX - debugBackdropPadding,
      y: debugBounds.minY - debugBackdropPadding,
      width: (debugBounds.maxX - debugBounds.minX) + debugBackdropPadding * 2,
      height: (debugBounds.maxY - debugBounds.minY) + debugBackdropPadding * 2
    };
    const showDebugRecenter = Math.abs(panOffset.x) > 10 || Math.abs(panOffset.y) > 10;

    return (
      <div
        className="relative h-screen w-full overflow-hidden bg-[#070807] text-white cursor-grab active:cursor-grabbing"
        onMouseDown={handlePointerDown} onMouseMove={handlePointerMove} onMouseUp={handlePointerUp} onMouseLeave={handlePointerUp}
        onTouchStart={handlePointerDown} onTouchMove={handlePointerMove} onTouchEnd={handlePointerUp}
        onWheel={handleWheel}
      >
        <svg width="100%" height="100%" className="absolute inset-0 z-0 bg-[#10140f]">
          <defs>
            <pattern id="debug-forest-fallback" width="120" height="120" patternUnits="userSpaceOnUse" patternTransform={`scale(${zoom})`}>
              <rect width="120" height="120" fill="#10140f" />
              <circle cx="30" cy="30" r="25" fill="#182016" />
              <circle cx="90" cy="50" r="35" fill="#0c120b" />
              <circle cx="40" cy="100" r="20" fill="#1b2518" />
              <circle cx="100" cy="110" r="15" fill="#23301f" />
            </pattern>
            <pattern id="debug-asphalt" width="60" height="60" patternUnits="userSpaceOnUse" patternTransform={`scale(${zoom})`}>
            ...
            <pattern id="debug-red-asphalt" width="60" height="60" patternUnits="userSpaceOnUse" patternTransform={`scale(${zoom})`}>
            ...
            <pattern
              id="debug-terrain-backdrop"
              x={debugBounds.center.x - TERRAIN_BACKDROP_SIZE / 2}
              y={debugBounds.center.y - TERRAIN_BACKDROP_SIZE / 2}
              width={TERRAIN_BACKDROP_SIZE}
              height={TERRAIN_BACKDROP_SIZE}
              patternUnits="userSpaceOnUse"
            >
              <image
                href={COURSE_BACKGROUNDS.hakone}
                width={TERRAIN_BACKDROP_SIZE}
                height={TERRAIN_BACKDROP_SIZE}
          ...
          <rect width="100%" height="100%" fill="url(#debug-forest-fallback)" />

          <g transform={`translate(${winSize.w / 2}, ${winSize.h / 2}) scale(${zoom}) translate(${-debugBounds.center.x + panOffset.x}, ${-debugBounds.center.y + panOffset.y})`} style={{ transition: isDragging ? 'none' : 'transform 0.3s cubic-bezier(0.25, 1, 0.5, 1)' }}>
            <rect
              x={debugBackdropBounds.x}
              y={debugBackdropBounds.y}
              width={debugBackdropBounds.width}
              height={debugBackdropBounds.height}
              fill="url(#debug-terrain-backdrop)"
              opacity="0.92"
            ...
            {roadDebugLayout.groups.map(group => (
              <g key={group.card.id} transform={`translate(${group.offset.x}, ${group.offset.y})`}>
                {[0, 1].map(lane =>
                  group.track[lane].map((space, index) => (
                    <g key={`debug-base-${group.card.id}-${lane}-${index}`}>
                      <path d={space.path} fill={space.limit ? 'url(#debug-red-asphalt)' : 'url(#debug-asphalt)'} stroke="#d4d4d8" strokeWidth="2" />
                ...
                {group.track[0].filter(space => space.roadCardStart).map(space => (
                  <g key={`debug-card-label-${group.card.id}`} transform={`translate(${space.center.x}, ${space.center.y}) rotate(${space.angle})`} className="pointer-events-none">
                    <g transform="translate(-35, 58)">
                      <rect x="-110" y="-18" width="220" height="36" rx="8" fill="rgba(9,9,11,0.9)" stroke="#facc15" strokeWidth="2" />
                      <text fill="#facc15" fontSize="13" fontWeight="900" fontFamily="sans-serif" textAnchor="middle" dy="5">
                        {String(space.roadCardStart.index).padStart(2, '0')} / {space.roadCardStart.name}
                      </text>
                    </g>
                  </g>
                ))}
                ...
                {[0, 1].map(lane =>
                  group.track[lane].filter(space => space.limit).map((space, index) => {
                    const xOffset = space.id.includes('-L') ? -65 : 65;
                    return (
                      <g key={`debug-limit-${group.card.id}-${lane}-${index}`} transform={`translate(${space.center.x}, ${space.center.y}) rotate(${space.angle})`}>
                        <g transform={`translate(${xOffset}, 0)`}>
                          ...
                          <text fill="#000" fontSize="24" fontWeight="900" fontFamily="sans-serif" textAnchor="middle" dy="8">{space.limit}</text>
                        </g>
                      </g>
                    );
                  })
        ...
        <button
          ...
          className="fixed left-4 top-4 z-[90] flex items-center gap-2 rounded-full border border-white/15 bg-zinc-950/80 px-4 py-2 text-xs font-black uppercase tracking-[0.18em] text-zinc-200 shadow-2xl backdrop-blur-md transition hover:border-yellow-300 hover:bg-zinc-900 hover:text-yellow-200 no-pan"
        >
          <ChevronLeft size={16} />
          {t('ui.road_cards', 'Road Cards')}
        </button>

        <div className="fixed left-1/2 top-16 z-[80] max-w-[calc(100vw-2rem)] -translate-x-1/2 rounded-full border border-emerald-300/35 bg-zinc-950/80 px-5 py-2 text-center shadow-2xl backdrop-blur-md no-pan sm:top-4">
          <div className="text-[10px] font-black uppercase tracking-[0.35em] text-emerald-300">{t('ui.debug_map', 'Debug Map')}</div>
          <div className="text-xs font-bold uppercase tracking-widest text-zinc-300">{t('ui.cards_label', '{count} Cards', { count: roadDebugLayout.groups.length })} / {t('ui.race_scale_geometry', 'race-scale geometry')}</div>
        </div>
        ...
        {showDebugRecenter && (
          <button
            ...
            className="fixed bottom-5 left-1/2 z-[90] flex -translate-x-1/2 items-center gap-2 rounded-full border border-emerald-300/40 bg-emerald-500 px-5 py-2 text-xs font-black uppercase tracking-widest text-black shadow-2xl transition hover:bg-emerald-300 no-pan"
          >
            <Crosshair size={16} />
            {t('ui.recenter', 'Recenter')}
          </button>
        )}
      </div>
    );
  ...
  if (appState === 'CODEX') {
    const activeList = codexTab === 'racing' ? racingCodexCards : ROAD_CARDS;
    const activeItem = activeList.find(item => item.id === (codexLockedId || codexHoveredId)) || activeList[0];
    const selectedId = activeItem?.id;
    const groupedCards = activeList.reduce((groups, card) => {
      const key = codexTab === 'racing' ? (card.deckCode || 'CARDS') : 'ROAD';
      return { ...groups, [key]: [...(groups[key] || []), card] };
    }, {});
    const renderCodexGridCard = (card) => {
      ...
      return (
        <button
          key={card.id}
          onMouseEnter={() => !codexLockedId && setCodexHoveredId(card.id)}
          onClick={() => setCodexLockedId(card.id)}
          aria-label={`${card.name} codex card`}
          className={`group relative aspect-[23/32] overflow-hidden rounded-md border-2 bg-zinc-950 shadow-lg transition hover:-translate-y-1 hover:border-white hover:shadow-[0_0_18px_rgba(250,204,21,0.2)] ${isActive ? 'border-yellow-300 ring-2 ring-yellow-300/70' : codexTab === 'racing' ? typeTone : 'border-zinc-600'}`}
        >
          {codexTab === 'racing' && card.imageSrc ? (
            <img src={card.imageSrc} alt="" draggable="false" className="absolute inset-0 h-full w-full object-cover" />
          ) : (
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_14%,rgba(250,204,21,0.22),transparent_34%),linear-gradient(145deg,rgba(39,39,42,0.95),rgba(5,5,5,1))]">
              <div className="absolute inset-0 opacity-30 bg-[repeating-linear-gradient(135deg,rgba(255,255,255,0.12)_0px,rgba(255,255,255,0.12)_1px,transparent_1px,transparent_10px)]" />
              <RoadSegmentSvg segments={card.segments} entrySpaces={card.entrySpaces} exitSpaces={card.exitSpaces} viewWidth={120} viewHeight={168} showSigns={false} className="absolute inset-1 h-[calc(100%-0.5rem)] w-[calc(100%-0.5rem)]" />
            </div>
          )}
          {codexTab === 'racing' && !card.imageSrc && (
          ...
          {isActive && <div className="absolute inset-0 bg-yellow-300/10 mix-blend-screen" />}
        </button>
      );
    };

    return (
      <div onClick={() => setCodexLockedId(null)} className="h-screen w-full overflow-hidden bg-[#070807] text-white">
        <div className="flex h-full flex-col bg-[radial-gradient(circle_at_70%_15%,rgba(250,204,21,0.12),transparent_34%),linear-gradient(135deg,rgba(127,29,29,0.16),transparent_38%,rgba(24,24,27,0.72))]">
          <header className="flex items-center justify-between border-b border-white/15 px-5 py-4 sm:px-8">
            ...
            <div className="text-center">
              <div className="text-3xl font-black italic tracking-tight sm:text-4xl">CODEX</div>
              <div className="text-[10px] font-bold uppercase tracking-[0.35em] text-yellow-400">{t('ui.card_archive', 'Card Archive')}</div>
            </div>
            <div className="hidden rounded border border-white/20 bg-black/40 px-3 py-2 text-xs font-black uppercase text-zinc-300 sm:block">{t('ui.cards_label', '{count} Cards', { count: activeList.length })}</div>
          </header>

          <main className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[22rem_1fr]">
            <aside className="min-h-0 overflow-y-auto border-b border-white/15 bg-black/65 px-4 py-5 lg:border-b-0 lg:border-r lg:px-5" onClick={() => setCodexLockedId(null)}>
              {activeItem && (
                <div>
                  <div className="flex justify-center">
                    {codexTab === 'racing'
                      ? <RenderCard card={activeItem} scale={1.12} extraClasses="cursor-default origin-top" />
                      : <div className="origin-top scale-[1.12]"><RoadCardPreview card={activeItem} /></div>}
                  </div>
                  <div className="mt-7">
                    <div className="text-[10px] font-black uppercase tracking-[0.35em] text-yellow-400">{codexTab === 'racing' ? activeItem.deckCode : t('ui.road_cards', 'Road Card')}</div>
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-2">
                      {codexTab === 'racing' ? (
                        <>
                          ...
                          <span className="border border-white/15 bg-black/50 px-3 py-2 text-xs font-black uppercase text-zinc-300">{activeItem.serialNumber}</span>
                        </>
                      ) : (
                        <>
                          <span className="border border-white/15 bg-black/50 px-3 py-2 text-xs font-black uppercase text-zinc-300">{t('ui.entry', 'Entry {count}', { count: activeItem.entrySpaces })}</span>
                          <span className="border border-white/15 bg-black/50 px-3 py-2 text-xs font-black uppercase text-zinc-300">{t('ui.exit', 'Exit {count}', { count: activeItem.exitSpaces })}</span>
                          <span className="border border-white/15 bg-black/50 px-3 py-2 text-xs font-black uppercase text-zinc-300">{t('ui.weight', 'Weight {count}', { count: activeItem.weight })}</span>
                        </>
                      )}
                    </div>
                    ...
                    {codexTab === 'racing' ? (
                      <>
                        <div className="mt-6 border-l-4 border-yellow-400 bg-black/45 p-4">
                      ...
                      <div className="mt-6 space-y-3">
                        {activeItem.segments.map((segment, index) => (
                          <div key={index} className="border border-white/10 bg-black/45 p-4">
                            <div className="text-[10px] font-black uppercase tracking-[0.28em] text-zinc-500">{t('ui.segment', 'Segment {count}', { count: index + 1 })}</div>
                            <div className="mt-2 text-base font-black uppercase text-white">{segment.type === 'straight' ? t('ui.straight_spaces', 'Straight / {count} spaces', { count: segment.spaces }) : t('ui.degree_curve', '{angle} degree {direction} curve', { angle: segment.angle, direction: segment.direction })}</div>
                            {segment.type === 'curve' && <div className="mt-1 text-xs font-bold text-zinc-400">{t('ui.radius_inner_outer_limit', 'Radius {radius} / Inner {inner} spaces / Outer {outer} spaces / Speed limit {limit} km/h', { radius: segment.radius, inner: segment.in_spaces, outer: segment.out_spaces, limit: segment.speed_limit })}</div>}
                          </div>
                        ))}
                      </div>
                    )}

                    {codexLockedId && <div className="mt-6 text-[10px] font-black uppercase tracking-[0.25em] text-yellow-400">{t('ui.selection_locked', 'Selection locked')}</div>}
                  </div>
                </div>
              )}
            </aside>
            ...
            <section className="flex min-h-0 flex-col bg-black/30" onClick={event => event.stopPropagation()}>
              <div className="grid grid-cols-2 gap-2 border-b border-white/15 bg-zinc-950/65 p-3 lg:grid-cols-[1fr_1fr_auto]">
                {[
                  ['racing', t('ui.racing_cards', 'Racing Cards')],
                  ['road', t('ui.road_cards', 'Road Cards')]
                ].map(([tab, label]) => (
                  <button
                    key={tab}
                    onClick={() => {
                    ...
                    className={`px-3 py-3 text-xs font-black uppercase tracking-wider transition ${codexTab === tab ? 'bg-yellow-400 text-black' : 'border border-white/15 bg-black text-zinc-400 hover:border-yellow-400 hover:text-white'}`}
                  >
                    {label}
                  </button>
                ))}
                {codexTab === 'road' && (
                  <button
                    type="button"
                    onClick={openRoadDebugMap}
                    className="col-span-2 flex items-center justify-center gap-2 border-2 border-emerald-300 bg-emerald-400 px-4 py-3 text-xs font-black uppercase tracking-wider text-black shadow-[0_0_18px_rgba(52,211,153,0.25)] transition hover:bg-emerald-300 lg:col-span-1"
                  >
                    <MapIcon size={16} />
                    {t('ui.debug_map', 'Debug Map')}
                  </button>
                )}
              </div>
              <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-5">
                {Object.entries(groupedCards).map(([group, cards]) => (
                  <div key={group} className="mb-7">
                    ...
                    <div className="grid grid-cols-4 gap-3 sm:grid-cols-5 md:grid-cols-6 xl:grid-cols-8 2xl:grid-cols-10">
                      {cards.map(renderCodexGridCard)}
                    </div>
                  </div>
                ))}
              </div>
  ...
  if (appState === 'SELECT') {
    ...
    const CarPortrait = ({ car, player }) => (
      <div className={`relative h-36 sm:h-44 overflow-hidden border-b-4 ${player === 0 ? 'border-red-500 bg-gradient-to-br from-red-950 to-zinc-950' : 'border-blue-500 bg-gradient-to-bl from-blue-950 to-zinc-950'}`}>
        ...
        <div className={`absolute inset-0 ${player === 0 ? 'bg-[radial-gradient(circle_at_72%_45%,rgba(248,113,113,0.32),transparent_42%)]' : 'bg-[radial-gradient(circle_at_28%_45%,rgba(96,165,250,0.32),transparent_42%)]'}`} />
        {car ? (
          <>
            <img
              src={car.cutout}
              alt={`${car.name} mugshot portrait`}
              className="absolute inset-0 h-full w-full object-contain object-center saturate-125 contrast-110 drop-shadow-[0_18px_18px_rgba(0,0,0,0.65)]"
              style={{ transform: player === 1 ? 'scaleX(-1) scale(1.42)' : 'scale(1.42)' }}
            />
      ...
      <div className="h-screen w-full overflow-y-auto bg-[#090b08] text-white selection:bg-yellow-400 selection:text-black">
        <div className="min-h-screen bg-[radial-gradient(circle_at_50%_20%,rgba(91,108,59,0.32),transparent_45%),linear-gradient(120deg,rgba(127,29,29,0.16),transparent_38%,rgba(30,58,138,0.16))] px-4 py-4 sm:px-6 lg:px-10">
          <header className="mx-auto flex max-w-7xl items-center justify-between border-b border-white/15 pb-3">
            ...
            <div className="text-center"><div className="text-2xl sm:text-4xl font-black italic tracking-tight">{t('ui.race_setup', 'RACE SETUP')}</div><div className="text-[10px] font-bold tracking-[0.35em] text-yellow-400">{t('ui.course_machine_select', 'COURSE & MACHINE SELECT')}</div></div>
            <div className="flex items-center gap-3">
              <button
                type="button"
                disabled={!canEditCourse}
                onClick={() => canEditCourse && setTutorialEnabled(enabled => !enabled)}
                className={`flex items-center gap-3 border-2 border-yellow-300 px-4 py-2 text-xs font-black uppercase tracking-widest shadow-[0_0_18px_rgba(250,204,21,0.22)] transition ${!canEditCourse ? 'cursor-not-allowed bg-zinc-900 text-zinc-600 opacity-60' : tutorialEnabled ? 'bg-yellow-400 text-black' : 'bg-black/60 text-yellow-200 hover:bg-yellow-400 hover:text-black'}`}
                aria-pressed={tutorialEnabled}
              >
                <span className={`h-4 w-8 border p-0.5 transition ${tutorialEnabled ? 'border-black/40 bg-black/20' : 'border-yellow-300/60 bg-zinc-950'}`}>
                  <span className={`block h-full w-3 bg-current transition-transform ${tutorialEnabled ? 'translate-x-3' : 'translate-x-0'}`} />
                </span>
                {tutorialEnabled ? t('ui.tutorial_on', 'Tutorial On') : t('ui.tutorial_off', 'Tutorial Off')}
              </button>
              <div className="rounded border border-white/20 bg-black/40 px-3 py-2 text-xs font-black uppercase text-zinc-300">{gameMode === 'PvE' ? '1P vs CPU' : gameMode === 'OnlinePvP' ? t('ui.online_2_player', 'Online 2P') : t('ui.local_2_player', 'Local 2P')}</div>
            </div>
          </header>
          ...
          <main className="mx-auto grid max-w-7xl gap-4 py-4 lg:grid-rows-[minmax(270px,42vh)_minmax(300px,1fr)]">
            <section className="grid overflow-hidden border border-white/15 bg-black/60 shadow-2xl lg:grid-cols-[1.25fr_1fr]">
              <div className="relative min-h-52 border-b border-white/15 p-5 lg:border-b-0 lg:border-r">
                ...
                <div className="absolute bottom-4 right-4 flex gap-2">
                  {selectedCourse.id === 'random' ? Object.keys(RANDOM_SIZE_CARDS).map(size => <button key={size} disabled={!canEditCourse} onClick={() => canEditCourse && setRandomMapSize(size)} className={`px-3 py-2 text-xs font-black uppercase ${!canEditCourse ? 'cursor-not-allowed opacity-50' : ''} ${randomMapSize === size ? 'bg-yellow-400 text-black' : 'border border-white/20 bg-black/70 text-zinc-300 hover:border-yellow-400'}`}>{size}<span className="ml-1 text-[9px] opacity-60">{t('ui.cards_count', '{count} cards', { count: RANDOM_SIZE_CARDS[size] })}</span></button>) : ['downhill', 'uphill'].map(direction => <button key={direction} disabled={!canEditCourse} onClick={() => canEditCourse && setCourseDirection(direction)} className={`px-4 py-2 text-xs font-black uppercase ${!canEditCourse ? 'cursor-not-allowed opacity-50' : ''} ${courseDirection === direction ? 'bg-yellow-400 text-black' : 'border border-white/20 bg-black/70 text-zinc-300 hover:border-yellow-400'}`}>{direction === 'downhill' ? t('ui.down', 'DOWN') : t('ui.up', 'UP')}</button>)}
                </div>
              </div>
            ...
            <section className="grid gap-3 lg:grid-cols-[1fr_1.25fr_1fr]">
              ...
              <div className="grid grid-cols-2 gap-2 self-stretch lg:col-start-2 lg:row-start-1">
                {localizedCarOptions.map((car, index) => {
                  ...
                  return <button key={car.id} onClick={() => chooseCar(car.id)} className={`group relative min-h-36 overflow-hidden border bg-zinc-950 p-3 text-left transition hover:-translate-y-1 hover:border-yellow-400 ${pickedBy.length ? 'border-yellow-500/60' : 'border-white/15'}`}>
                    <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_48%,rgba(250,204,21,0.18),transparent_42%)]" />
  ...
  const terrainBackgroundImage = gameState?.raceSetup?.backgroundImage || COURSE_BACKGROUNDS.akagi;
  const terrainBackdropBounds = {
    x: trackBounds.minX - TERRAIN_BACKDROP_PADDING,
    y: trackBounds.minY - TERRAIN_BACKDROP_PADDING,
    width: (trackBounds.maxX - trackBounds.minX) + TERRAIN_BACKDROP_PADDING * 2,
    height: (trackBounds.maxY - trackBounds.minY) + TERRAIN_BACKDROP_PADDING * 2
  };
  ...
  const activeMovePointsLeft = gameState?.phase === 'NN_MOVE'
    ? (gameState.nnMpLeft?.[activePlayerId] || 0)
  ...
  const visiblePilePlayer = isOnlineGame ? localPlayer : gameMode === 'PvE' ? gameState.players[0] : activePlayer;
  ...
  const visibleHandPlayer = isOnlineGame ? localPlayer : gameMode === 'PvE' ? gameState.players[0] : activePlayer;
  const shouldShowLocalHand = gameMode === 'PvE' ? true : !isAITurn && (!isOnlineGame || isLocalPlayersTurn);
  ...
  const activeHandCanPlay = !isAITurn && isLocalPlayersTurn && (gameState.phase === 'PLAY_CARD' || gameState.phase === 'NN_SELECT_CARD');
  const activeHandTucked = !activeHandCanPlay;
  const activeHandLabel = activeHandCanPlay
    ? gameState.phase === 'NN_SELECT_CARD' ? t('ui.lock_card_face_down', 'Lock in 1 Card Face Down!') : t('ui.play_driving_card', 'Play 1 Driving Card')
    : isAITurn ? t('ui.hand_locked_rival', 'Hand Locked During Rival Turn') : t('ui.hand_locked_movement', 'Hand Locked During Movement');
    ...
    >
      ...
      <style>{`
        ...
        @keyframes turn-banner-pop {
          0% { transform: translateY(-28px) scale(0.9); opacity: 0; filter: blur(8px); }
        ...
        @keyframes movement_warning_breathe {
          0%, 100% {
            opacity: 0.68;
            filter: drop-shadow(0 0 5px rgba(250, 204, 21, 0.58));
          }
          50% {
            opacity: 1;
        ...
        @keyframes tutorial-glow {
          0%, 100% { box-shadow: 0 0 0 3px rgba(250,204,21,0.95), 0 0 28px rgba(250,204,21,0.45); }
          50% { box-shadow: 0 0 0 6px rgba(250,204,21,0.7), 0 0 44px rgba(250,204,21,0.72); }
        }
        @keyframes tutorial-svg-glow {
          0%, 100% { opacity: 0.35; }
          50% { opacity: 0.75; }
        }
        @keyframes deck-card-draw-to-hand {
          0% { opacity: 0; transform: translate(0, 0) scale(0.72) rotate(-8deg); }
          14% { opacity: 1; }
          78% { opacity: 1; transform: translate(-40vw, -9vh) scale(1.04) rotate(6deg); }
          100% { opacity: 0; transform: translate(-46vw, -12vh) scale(1.08) rotate(8deg); }
        }
        .tutorial-highlight {
          animation: tutorial-glow 1.2s ease-in-out infinite;
        }
        .tutorial-svg-highlight {
          animation: tutorial-svg-glow 1.2s ease-in-out infinite;
        }
      ...
      <svg width="100%" height="100%" className="absolute inset-0 z-0 bg-[#1e2418]">
        <defs>
          ...
          <pattern
            id="terrain-backdrop"
            x={trackBounds.center.x - TERRAIN_BACKDROP_SIZE / 2}
            y={trackBounds.center.y - TERRAIN_BACKDROP_SIZE / 2}
            width={TERRAIN_BACKDROP_SIZE}
            height={TERRAIN_BACKDROP_SIZE}
            patternUnits="userSpaceOnUse"
          >
            <image
              href={terrainBackgroundImage}
              width={TERRAIN_BACKDROP_SIZE}
              height={TERRAIN_BACKDROP_SIZE}
              preserveAspectRatio="xMidYMid slice"
            />
        ...
        <g transform={`translate(${winSize.w/2 - 140}, ${winSize.h/2}) scale(${zoom}) translate(${-cameraTarget.x + panOffset.x}, ${-cameraTarget.y + panOffset.y})`} style={{ transition: isDragging ? 'none' : 'transform 0.3s cubic-bezier(0.25, 1, 0.5, 1)' }}>
          ...
          {/* Top-down terrain backdrop tiles with the road. */}
          <rect
            x={terrainBackdropBounds.x}
            y={terrainBackdropBounds.y}
            width={terrainBackdropBounds.width}
            height={terrainBackdropBounds.height}
            fill="url(#terrain-backdrop)"
            opacity="0.88"
            className="pointer-events-none"
          />
          <rect
            ...
            TRACK[lane].filter(s => s.limit).map((s, i) => {
               ...
               return (
                  <g key={`limit-${lane}-${i}`} transform={`translate(${s.center.x}, ${s.center.y}) rotate(${s.angle})`}>
                    <g transform={`translate(${xOffset}, 0)`}>
                      {activeTutorialTarget === 'speed-limit' && <circle r="36" fill="none" stroke="#facc15" strokeWidth="7" className="tutorial-svg-highlight" />}
                      <rect x="-4" y="0" width="8" height="35" fill="#9ca3af" stroke="#4b5563" strokeWidth="1" filter="drop-shadow(0 4px 4px rgba(0,0,0,0.5))" />
                      <circle r="26" fill="#fff" filter="drop-shadow(0 5px 5px rgba(0,0,0,0.5))" />
                      <circle r="22" fill="none" stroke="#dc2626" strokeWidth="6" />
          ...
          {validMovesList.map(m => {
            const space = TRACK[m.lane][m.idx];
            const isUndersteerDanger = willMoveUndersteer(activePlayer, m.lane, m.idx, TRACK);
            const isTutorialMovement = activeTutorialTarget === 'movement';
            const fill = isUndersteerDanger || isTutorialMovement ? 'rgba(250, 204, 21, 0.42)' : 'rgba(16, 185, 129, 0.42)';
            const stroke = isUndersteerDanger || isTutorialMovement ? 'rgba(250, 204, 21, 0.98)' : 'rgba(110, 231, 183, 0.95)';
            const animation = isUndersteerDanger ? 'movement_warning_breathe 1.45s ease-in-out infinite' : 'movement_breathe 1.45s ease-in-out infinite';

            return (
              <g key={`move-${m.lane}-${m.idx}`}>
                <path
                  d={space.path}
                  fill={fill}
                  stroke={stroke}
                  strokeWidth="8"
                  className={`cursor-pointer transition-colors ${isUndersteerDanger ? 'hover:fill-yellow-300/70' : 'hover:fill-emerald-300/70'} ${isTutorialMovement ? 'tutorial-svg-highlight' : ''}`}
                  style={{ animation }}
                  onClick={() => attemptMove(m.lane, m.idx)}
                />
                {isUndersteerDanger && (
                  <g
                    transform={`translate(${space.center.x}, ${space.center.y})`}
                    style={{ pointerEvents: 'none', animation }}
                  >
                    <path d="M 0 -28 L 30 24 L -30 24 Z" fill="rgba(250, 204, 21, 0.95)" stroke="#111827" strokeWidth="5" />
                    <path d="M 0 -28 L 30 24 L -30 24 Z" fill="none" stroke="rgba(255,255,255,0.8)" strokeWidth="2" />
                    <text x="0" y="13" fill="#111827" fontSize="36" fontWeight="900" fontFamily="sans-serif" textAnchor="middle">!</text>
                  </g>
                )}
              </g>
            );
          ...
          {gameState && gameState.players.map(p => {
            ...
            const lastCardType = p.lastPlayed ? CARDS[p.lastPlayed].rulesType : null;
            const isGas = lastCardType === 'Gas';
            const isTurn = lastCardType === 'Turn';
            const isBrake = lastCardType === 'Brake';
            const showCarMpBadge = canControlActivePlayer && p.id === activePlayerId && (gameState.phase === 'MOVE' || gameState.phase === 'NN_MOVE') && activeMovePointsLeft > 0;
            const carMpBadgeOffset = showCarMpBadge ? getRoadsideBadgeOffset(p, TRACK) : null;
            ...
            return (
              <g key={`car-${p.id}`} transform={`translate(${s.center.x}, ${s.center.y}) rotate(${s.angle})`} style={{ transition: 'transform 0.3s cubic-bezier(0.25, 1, 0.5, 1)' }} className="pointer-events-none">
                ...
                {activeTutorialTarget === 'active-car' && p.id === activePlayerId && (
                  <ellipse cx="0" cy="0" rx="48" ry="68" fill="rgba(250,204,21,0.18)" stroke="#facc15" strokeWidth="8" className="tutorial-svg-highlight" />
                )}

                {showCarMpBadge && (
                  <g
                    transform={`translate(${carMpBadgeOffset.x}, ${carMpBadgeOffset.y})`}
                    style={{ animation: 'movement_breathe 1.45s ease-in-out infinite' }}
                  >
                    <g transform={`rotate(${-s.angle})`}>
                      <rect x="-40" y="-34" width="80" height="68" rx="14" fill="rgba(6, 78, 59, 0.86)" stroke="rgba(52, 211, 153, 0.55)" strokeWidth="3" />
                      <text x="0" y="-2" fill="#6ee7b7" fontSize="34" fontWeight="900" fontFamily="sans-serif" textAnchor="middle">{activeMovePointsLeft}</text>
                      <text x="0" y="24" fill="rgba(209, 250, 229, 0.85)" fontSize="22" fontWeight="900" fontFamily="sans-serif" textAnchor="middle">MP</text>
                    </g>
                  </g>
                )}
      ...
      {gameState?.overtakeEvent && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 pointer-events-none w-full overflow-hidden flex flex-col items-center">
          <div className="bg-gradient-to-r from-transparent via-red-600 to-transparent w-full py-6 transform -skew-y-3 shadow-[0_0_50px_rgba(220,38,38,0.8)] flex items-center justify-center animate-in zoom-in duration-200">
            <h2 className="text-7xl font-black text-white italic tracking-tighter drop-shadow-2xl uppercase">{t('ui.overtake', 'OVERTAKE!')}</h2>
          </div>
          <div className="text-center mt-4 text-3xl font-black text-yellow-400 drop-shadow-[0_0_15px_rgba(251,191,36,0.8)] animate-bounce bg-black/50 px-8 py-2 rounded-full border border-yellow-500/30">
            {t('ui.takes_lead', '{player} takes the lead!', { player: gameState.overtakeEvent })}
          </div>
        </div>
      )}
      ...
      {gameState?.crashEvent && (
        <div className="absolute top-[40%] left-1/2 -translate-x-1/2 -translate-y-1/2 z-[130] pointer-events-none w-full flex flex-col items-center animate-in zoom-in-50 duration-100">
          ...
          <div className="mt-6 text-4xl font-black text-white drop-shadow-[0_0_25px_rgba(255,0,0,1)] animate-bounce bg-black/90 px-10 py-4 rounded-full border-2 border-red-500">
            {t('ui.suffers_penalty', '{player} suffers {penalty}', { player: gameState.crashEvent.name, penalty: gameState.crashEvent.penalty })}
          </div>
        </div>
      )}
      ...
      {turnBanner && (
        <div
          key={turnBanner.key}
          className="fixed inset-x-0 top-24 z-[125] pointer-events-none flex justify-center px-4"
          style={{ animation: 'turn-banner-pop 1.5s ease-out forwards' }}
        >
          <div className={`max-w-[calc(100vw-2rem)] rounded-full border-2 px-5 py-3 shadow-[0_0_32px_rgba(250,204,21,0.35)] backdrop-blur-md md:px-8 ${turnBanner.playerId === 0 ? 'border-red-400 bg-red-950/85' : 'border-blue-400 bg-blue-950/85'}`}>
            <div className="text-center text-[10px] font-black uppercase tracking-[0.35em] text-yellow-300">{turnBanner.label}</div>
            <div className={`mt-1 truncate text-center text-2xl font-black italic uppercase tracking-wide md:text-3xl ${turnBanner.playerId === 0 ? 'text-red-200' : 'text-blue-200'}`}>
              {t('ui.player_plays', '{player} Plays', { player: turnBanner.playerName })}
            </div>
          </div>
        </div>
      )}
      ...
      {gameState && (
        <>
          <Speedometer
            ...
            isTutorialTarget={activeTutorialTarget === 'speedometer-active' && activePlayerId === 0}
          />
          <Speedometer
            ...
            isTutorialTarget={activeTutorialTarget === 'speedometer-active' && activePlayerId === 1}
          />
        </>
      )}

      {gameState && (
        <div className={`fixed bottom-5 right-5 z-40 flex items-end gap-3 no-pan ${activeTutorialTarget === 'piles' ? 'tutorial-highlight' : ''}`}>
      ...
      <DrawAnimation event={gameState?.drawAnimation} />
          ...
          <div className="w-full max-w-5xl max-h-[88vh] overflow-hidden rounded-2xl border-2 border-orange-500 bg-zinc-950 shadow-[0_0_50px_rgba(249,115,22,0.25)]">
            <div className="flex items-center justify-between border-b border-zinc-800 px-5 py-4">
              <div>
                ...
                <h2 className="text-2xl font-black uppercase text-white">{t('ui.discard_pile', 'Discard Pile ({count})', { count: visiblePilePlayer.discard.length })}</h2>
              </div>
              <button onClick={() => setInspectDiscardOpen(false)} className="rounded-lg border border-zinc-700 bg-zinc-900 px-4 py-2 text-sm font-black uppercase text-zinc-300 hover:border-white hover:text-white">{t('ui.close', 'Close')}</button>
            </div>
            <div className="max-h-[70vh] overflow-y-auto p-5">
              {visiblePilePlayer.discard.length > 0 ? (
                ...
                <div className="py-16 text-center text-sm font-bold uppercase tracking-widest text-zinc-500">{t('ui.no_discarded_cards', 'No discarded cards')}</div>
              )}
            </div>
          </div>
        </div>
      ...
      {showRecenter && (
        <button onClick={() => setPanOffset({ x: 0, y: 0 })} className="fixed top-32 left-1/2 -translate-x-1/2 bg-blue-600 hover:bg-blue-500 text-white px-6 py-2 rounded-full font-bold shadow-2xl z-50 flex items-center gap-2 no-pan transition-all animate-bounce">
          <Crosshair size={18} /> {t('ui.recenter', 'Recenter')}
        </button>
      )}

      <TutorialOverlay step={activeTutorialStep} onDismiss={dismissTutorial} onSkip={skipTutorial} t={t} />
      ...
      {gameState?.isNNRound && (
         <div className="absolute top-6 left-1/2 -translate-x-1/2 bg-black/80 backdrop-blur border border-yellow-500 px-8 py-2 rounded-full shadow-2xl z-40 flex items-center gap-4 no-pan">
             <span className="text-yellow-500 font-black italic tracking-widest uppercase">{t('ui.neck_and_neck', 'Neck and Neck')}</span>
             {gameState.phase === 'NN_MOVE' && (
        ...
        <div className="absolute inset-0 z-[120] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 no-pan pointer-events-auto">
          <div className="w-full max-w-4xl max-h-[96vh] overflow-y-auto bg-zinc-950 border-2 border-yellow-500 shadow-[0_0_60px_rgba(250,204,21,0.35)] rounded-2xl p-5 md:p-8 flex flex-col items-center">
            <div className="flex items-center gap-2 mb-5" aria-label={`Reveal step ${nnRevealStageIndex + 1} of 3`}>
              {[
                t('ui.lock_in', 'LOCK IN'),
                t('ui.reveal', 'REVEAL'),
                t('ui.move_order', 'MOVE ORDER')
              ].map((label, index) => (
            ...
            <div className="text-center mb-5">
              <div className="text-xs md:text-sm font-black tracking-[0.35em] text-yellow-500 uppercase mb-2">{t('ui.neck_and_neck', 'Neck and Neck')}</div>
              <h2 className="text-3xl md:text-5xl font-black italic text-white tracking-tight">
                {nnRevealStage === 'faceDown' ? t('ui.cards_locked_in', 'CARDS LOCKED IN') : nnRevealStage === 'revealed' ? t('ui.simultaneous_reveal', 'SIMULTANEOUS REVEAL') : t('ui.action_order_set', 'ACTION ORDER SET')}
              </h2>
              <p className="text-zinc-400 mt-2 text-sm">
                {nnRevealStage === 'faceDown' ? t('ui.both_commit_face_down', 'Both drivers commit face down.') : nnRevealStage === 'revealed' ? t('ui.cards_flip_together', 'Both cards flip together. Comparing pre-effect speed...') : t('ui.first_step_decided', 'The first movement step is decided before card effects resolve.')}
              </p>
            </div>
            <div className="flex flex-col sm:flex-row gap-5 md:gap-12 items-center justify-center pointer-events-none">
              <div className="flex flex-col items-center">
                ...
                <div key={`p0-${nnRevealStage}`} className={`nn-card-stage ${nnRevealStage === 'faceDown' ? 'nn-card-deal-left' : nnRevealStage === 'revealed' ? 'nn-card-reveal' : ''}`}>
                  {nnRevealStage === 'faceDown' ? <RenderCardBack playerId={0} /> : gameState.nnSelections[0] ? <RenderCard card={CARDS[gameState.nnSelections[0]]} scale={0.85} /> : <div className="w-48 h-64 border-2 border-dashed border-red-500/50 rounded-xl flex items-center justify-center text-red-500 font-bold bg-black/50">{t('ui.no_card', 'NO CARD')}</div>}
                </div>
              </div>
              ...
              <div className="flex flex-col items-center">
                ...
                <div key={`p1-${nnRevealStage}`} className={`nn-card-stage ${nnRevealStage === 'faceDown' ? 'nn-card-deal-right' : nnRevealStage === 'revealed' ? 'nn-card-reveal' : ''}`}>
                  {nnRevealStage === 'faceDown' ? <RenderCardBack playerId={1} /> : gameState.nnSelections[1] ? <RenderCard card={CARDS[gameState.nnSelections[1]]} scale={0.85} /> : <div className="w-48 h-64 border-2 border-dashed border-blue-500/50 rounded-xl flex items-center justify-center text-blue-500 font-bold bg-black/50">{t('ui.no_card', 'NO CARD')}</div>}
                </div>
              </div>
            </div>
            {nnRevealStage === 'decision' && (
              <>
                <div className={`nn-decision-pop mt-5 w-full max-w-xl border-2 rounded-xl px-5 py-3 text-center ${nnFirstMover.id === 0 ? 'border-red-500 bg-red-950/50' : 'border-blue-500 bg-blue-950/50'}`}>
                  <div className="text-xs font-black tracking-[0.3em] text-zinc-400 uppercase">{t('ui.first_movement_step', 'First Movement Step')}</div>
                  <div className={`text-2xl md:text-3xl font-black italic mt-1 ${nnFirstMover.id === 0 ? 'text-red-400' : 'text-blue-400'}`}>
                    {t('ui.moves_first', '{player} MOVES FIRST', { player: nnFirstMover.name })}
                  </div>
                  <div className="text-xs text-zinc-400 mt-1">{t('ui.then_alternates', 'Then movement alternates one point at a time.')}</div>
                </div>
                {gameMode !== 'OnlinePvP' || onlineSession.role === 'host' ? (
                  <button onClick={resolveNNReveal} className="nn-decision-pop mt-5 px-8 py-3 bg-yellow-400 hover:bg-yellow-300 active:bg-yellow-500 text-black text-lg font-black uppercase tracking-wider rounded-lg shadow-[0_0_25px_rgba(250,204,21,0.4)] transition-colors">
                    {t('ui.close_banner_start_movement', 'Close Banner & Start Movement')}
                  </button>
                ) : (
                  <div className="nn-decision-pop mt-5 rounded-full border border-zinc-700 bg-black/70 px-6 py-3 text-sm font-black uppercase tracking-widest text-zinc-300">
                    {t('ui.waiting_host_movement', 'Waiting for host to start movement')}
                  </div>
                )}
              </>
            )}
      ...
      {/* Prompts Overlay */}
      <div className={`absolute top-1/4 left-1/2 -translate-x-1/2 z-30 pointer-events-none flex flex-col items-center no-pan ${activeTutorialTarget === 'prompt' ? 'tutorial-highlight' : ''}`}>
        {isAITurn ? (
           <div className="bg-black/80 px-6 py-3 rounded-full border border-zinc-700 flex items-center gap-3 animate-pulse shadow-2xl"><Bot className="text-blue-400" /><span className="font-bold text-lg">{t('ui.ai_thinking', 'AI is thinking...')}</span></div>
        ) : showBonusTurnPrompt ? (
           <div className="bg-black/80 px-6 py-3 rounded-full border border-zinc-700 flex items-center gap-3 shadow-2xl"><FastForward className="text-yellow-300" /><span className="font-bold text-lg">{t('ui.bonus_turn', 'Bonus turn')}</span></div>
           ...
           <div className="bg-black/80 px-6 py-3 rounded-full border border-zinc-700 flex items-center gap-3 shadow-2xl"><Radio className="text-blue-300" /><span className="font-bold text-lg">{t('ui.waiting_for_player', 'Waiting for {player}', { player: activePlayer.name })}</span></div>
        ) : (
          <>
            {canControlActivePlayer && gameState.phase === 'SLIPSTREAM' && (
              <div className="bg-black/90 p-6 rounded-2xl border border-zinc-700 shadow-2xl text-center pointer-events-auto">
                <h3 className="text-xl font-black text-yellow-400 mb-4">{t('ui.slipstream_available', 'SLIPSTREAM AVAILABLE')}</h3>
                <div className="flex gap-4">
                  <button onClick={() => acceptSlipstream(true)} className="px-6 py-3 bg-blue-600 hover:bg-blue-500 rounded-lg font-bold">{t('ui.take_slipstream', 'Take (+10 km/h)')}</button>
                  <button onClick={() => acceptSlipstream(false)} className="px-6 py-3 bg-zinc-700 hover:bg-zinc-600 rounded-lg font-bold">{t('ui.refuse', 'Refuse')}</button>
                </div>
              </div>
            )}
            {canControlActivePlayer && gameState.phase === 'CARD_CHOICE' && (
              <div className="bg-black/90 p-6 rounded-2xl border border-yellow-500 shadow-2xl text-center pointer-events-auto min-w-[360px]">
                <h3 className="text-xl font-black text-yellow-400 mb-2">
                  {activeChoiceType === 'torque_split' ? t('ui.torque_split', 'TORQUE SPLIT') : activeChoiceType === 'micro_correction' ? t('ui.micro_correction', 'MICRO CORRECTION') : t('ui.change_shift', 'CHANGE SHIFT')}
                </h3>
                <p className="mb-4 text-sm font-bold text-zinc-400">
                  {activeChoiceType === 'torque_split' ? t('ui.choose_speed_or_check', 'Choose speed now or corner limit this turn.') : activeChoiceType === 'micro_correction' ? t('ui.choose_speed_adjustment', 'Choose speed adjustment after the speed check.') : t('ui.choose_shift_direction', 'Choose shift direction.')}
                </p>
                <div className="flex flex-wrap justify-center gap-4">
                  {activeChoiceType === 'torque_split' ? (
                    <>
                      <button onClick={() => resolveCardChoice('speed')} className="px-6 py-3 bg-yellow-500 hover:bg-yellow-400 rounded-lg font-black text-black">{t('ui.speed_plus', 'Speed +{amount}', { amount: 30 })}</button>
                      <button onClick={() => resolveCardChoice('check_bonus')} className="px-6 py-3 bg-blue-600 hover:bg-blue-500 rounded-lg font-black text-white">{t('ui.check_plus', 'Check +{amount}', { amount: 40 })}</button>
                    </>
                  ) : (
                    <>
                      <button onClick={() => resolveCardChoice('speed_up')} className="px-6 py-3 bg-yellow-500 hover:bg-yellow-400 rounded-lg font-black text-black">{t('ui.speed_plus', 'Speed +{amount}', { amount: 10 })}</button>
                      <button onClick={() => resolveCardChoice('speed_down')} className="px-6 py-3 bg-red-700 hover:bg-red-600 rounded-lg font-black text-white">{t('ui.speed_minus', 'Speed -{amount}', { amount: 10 })}</button>
                    </>
                  )}
                </div>
              </div>
      ...
      {/* Fanned Cards UI */}
      {activeHandVisible && (
        <div className={`pointer-events-none absolute ${activeHandTucked ? '-bottom-28' : 'bottom-8'} left-[40%] -translate-x-1/2 flex items-end justify-center z-40 w-full no-pan transition-all duration-300`} style={{ perspective: '1200px' }}>
          <div className="relative flex justify-center h-64 items-end w-full max-w-[1200px]">
             {activeTutorialTarget === 'hand' && <div className="pointer-events-none absolute bottom-[-2px] left-1/2 h-[270px] w-[min(660px,82vw)] -translate-x-1/2 rounded-xl border-2 border-yellow-300 shadow-[0_0_34px_rgba(250,204,21,0.55)] tutorial-svg-highlight" />}
             ...
             {visibleHandPlayer.hand.map((cardId, idx) => {
               ...
               const transX = offset * 130;
               const isHovered = hoveredCardIdx === idx && (playable || activeHandTucked);

               return (
                 <div
                    key={idx}
                    className="pointer-events-auto absolute bottom-0"
                    onMouseEnter={() => (playable || activeHandTucked) && setHoveredCardIdx(idx)}
                    onMouseLeave={() => setHoveredCardIdx(null)}
                    style={{
                      transform: isHovered
                         ? `translateX(${transX}px) translateY(${activeHandTucked ? -125 : -40}px) scale(1.1) rotate(0deg)`
                 ...
                 >
                   <RenderCard
                     card={card}
                     disabled={!activeHandCanPlay || !playable}
                     onClick={playable ? () => playCard(cardId, idx) : undefined}
                     extraClasses={activeHandTucked ? 'opacity-90 grayscale-[0.15]' : ''}
                   />
                 </div>
               );
             })}
          </div>
          {activeHandCanPlay && <button onClick={skipToDiscard} className="pointer-events-auto absolute -bottom-2 right-[25%] px-9 py-6 bg-red-950/80 hover:bg-red-900 border border-red-900 rounded-lg text-m font-bold text-red-200 transition-colors shadow-2xl">{t('ui.skip_refresh', 'Skip/Refresh')}</button>}
        </div>
      )}
      ...
      {canControlActivePlayer && gameState.phase === 'DISCARD' && (
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center justify-end z-40 w-full no-pan max-w-4xl h-64">
             <div className="absolute -top-16 bg-red-900/90 px-6 py-2 rounded-full border border-red-500 text-sm font-bold text-white shadow-2xl animate-pulse">
                {t('ui.select_discard_optional', 'Select cards to discard (Optional)')}
             </div>
             ...
             <button onClick={confirmDiscard} className="px-10 py-3 bg-blue-600 hover:bg-blue-500 rounded-xl text-lg font-black text-white transition-colors shadow-2xl border-2 border-blue-400">
                {t('ui.confirm_discard', 'Confirm Discard')}
             </button>
        </div>
      )}
        ...
        <div className="absolute inset-0 z-[110] bg-black/65 backdrop-blur-sm flex items-end justify-center p-6 no-pan">
          <div className="w-full max-w-5xl bg-zinc-950/95 border-2 border-emerald-500 rounded-2xl shadow-[0_0_40px_rgba(16,185,129,0.25)] p-5 flex flex-col items-center">
            <div className="text-center mb-4">
              <div className="text-xs font-black tracking-[0.3em] text-emerald-400 uppercase">{activeCycleRequired ? t('ui.required_discard', 'Required Discard') : t('ui.optional_hand_cycling', 'Optional Hand Cycling')}</div>
              <h3 className="text-xl font-black text-white mt-1">{activePlayer.name}: {activeCycleRequired ? t('ui.discard_n_card', 'discard {count} card', { count: activeCycleNeeded }) : t('ui.discard_up_to_n_card', 'discard up to {count} card', { count: activeCycleMax })}</h3>
              <p className="text-sm text-zinc-400 mt-1">{t('ui.selected_refills', 'Selected cards go to your discard pile. Your hand refills at turn end.')}</p>
            </div>
            ...
            <button onClick={() => confirmCardCycle()} disabled={activeCycleRequired && discardSelection.length < activeCycleNeeded} className={`px-8 py-3 rounded-lg text-base font-black uppercase tracking-wider transition-colors ${activeCycleRequired && discardSelection.length < activeCycleNeeded ? 'bg-zinc-800 text-zinc-500 cursor-not-allowed' : 'bg-emerald-500 hover:bg-emerald-400 active:bg-emerald-600 text-black'}`}>
              {discardSelection.length > 0 ? t('ui.discard_continue', 'Discard {count} & Continue', { count: discardSelection.length }) : activeCycleRequired ? t('ui.select_card', 'Select Card') : t('ui.keep_hand_continue', 'Keep Hand & Continue')}
            </button>
          </div>
        </div>
      )}
      ...
      <div className={`fixed right-0 top-0 h-full w-80 bg-zinc-950/95 border-l border-zinc-800 shadow-2xl transition-transform duration-300 z-50 flex flex-col no-pan ${isRadioOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        ...
        <div className="p-6 border-b border-zinc-800 bg-zinc-900/50">
          <h2 className="text-lg font-black uppercase tracking-widest flex items-center gap-2 text-zinc-100"><Radio size={20} className="text-green-500" />{t('ui.race_radio', 'Race Radio')}</h2>
        </div>
      ...
      {appState === 'GAMEOVER' && (
        <div className="absolute inset-0 bg-black/90 z-50 flex items-center justify-center flex-col backdrop-blur-sm no-pan">
          <h1 className="text-7xl font-black mb-4 text-transparent bg-clip-text bg-gradient-to-b from-yellow-300 to-yellow-600 drop-shadow-[0_0_20px_rgba(253,224,71,0.5)]">{t('ui.finish_wins', 'FINISH - {player} WINS!', { player: gameState.winner })}</h1>
          <button onClick={() => setAppState('TITLE')} className="mt-8 px-10 py-4 bg-white text-black text-xl font-black rounded-full hover:bg-zinc-200 transition-colors shadow-[0_0_20px_rgba(255,255,255,0.5)]">{t('ui.back_to_title', 'Back to Title')}</button>
        </div>
      )}
