...
import { Play, Users, Bot, ChevronRight, ChevronLeft, Radio, FastForward, Crosshair, BookOpen } from 'lucide-react';
...
const getUndersteerTarget = (lane, idx, track) => {
  ...
  const ahead = adjacent
    ...
    .sort((a, b) => track[outLane][a].distance - track[outLane][b].distance)[0];
  const fallback = adjacent
    .sort((a, b) => track[outLane][b].distance - track[outLane][a].distance)[0];

  return { lane: outLane, idx: ahead ?? fallback ?? idx };
};
...
const TUTORIAL_STEPS = [
  ...
  },
  {
    id: 'skip-turn',
    target: 'hand',
    title: 'Skip Turn and Dig',
    body: 'Skip Turn means you choose not to play a driving card this turn. That is not always wasted: it opens the discard step, lets you throw away cards you do not want, and helps dig through the deck. At the end of the turn, your hand draws back up to 4 cards.',
...
const TutorialOverlay = ({ step, onDismiss, onSkip }) => {
  if (!step) return null;
  return (
    <div className="fixed left-1/2 top-24 z-[150] w-[min(92vw,430px)] -translate-x-1/2 no-pan pointer-events-auto">
      <div className="border-2 border-yellow-300 bg-zinc-950/95 p-5 shadow-[0_0_40px_rgba(250,204,21,0.28)] backdrop-blur-md">
        <div className="text-[10px] font-black uppercase tracking-[0.35em] text-yellow-300">Tutorial</div>
        <h3 className="mt-2 text-2xl font-black italic uppercase text-white">{step.title}</h3>
        <p className="mt-3 text-sm font-bold leading-relaxed text-zinc-300">{step.body}</p>
        <div className="mt-5 flex flex-wrap items-center justify-end gap-3">
          <button onClick={onSkip} className="border border-zinc-700 bg-zinc-900 px-4 py-2 text-xs font-black uppercase tracking-widest text-zinc-400 transition hover:border-white hover:text-white">Skip Tutorial</button>
          <button onClick={onDismiss} className="bg-yellow-400 px-5 py-2 text-sm font-black uppercase tracking-wider text-black transition hover:bg-yellow-300">Got It</button>
        </div>
      </div>
    </div>
  );
...
export default function ArcadeRacingGame() {
  ...
  const ROAD_CARDS = useMemo(() => parseCornerCardRows(cornerCardRows), [cornerCardRows]);
  ...
  const attachPeerConnection = (connection) => {
    ...
    connection.on('open', () => {
      if (onlineRoleRef.current === 'host') {
        setOnlineSession(current => ({ ...current, guestConnected: true, message: 'Guest connected. Waiting for ready signal.' }));
        postOnlineMessage({ type: 'host-ack', roomKey: onlineRoomKeyRef.current, senderRole: 'host' });
      } else {
        setOnlineSession(current => ({ ...current, guestConnected: true, message: 'Connected to host. Mark ready when you are set.' }));
    ...
    connection.on('close', () => {
      ...
      setOnlineSession(current => ({ ...current, guestConnected: false, guestReady: false, message: 'Connection closed. Rejoin or host a new room.' }));
    });
    connection.on('error', () => {
      setOnlineSession(current => ({ ...current, message: 'Peer connection error. Check the room key or retry.' }));
    });
  };
  ...
  const startPeerNetworking = async (role, roomKey) => {
    try {
      ...
      peer.on('error', () => {
        setOnlineSession(current => ({ ...current, message: 'Online broker unavailable. Same-browser fallback is still active.' }));
      });
      if (role === 'guest') {
        peer.on('open', () => {
      ...
      setOnlineSession(current => ({ ...current, message: 'Online networking could not load. Same-browser fallback is still active.' }));
    }
  };
    ...
    const handleOnlineMessage = (message) => {
      ...
      if (onlineRoleRef.current === 'host') {
        ...
        if (message.type === 'guest-ready') {
          setOnlineSession(current => ({ ...current, guestConnected: true, guestReady: true, message: 'Guest is ready. You can start the race setup.' }));
        }
      }
  ...
  const createOnlineRoom = () => {
    ...
    setOnlineSession({
      ...
      role: 'host',
      roomKey,
      message: 'Share this room key with your guest.'
    });
  ...
  const joinOnlineRoom = () => {
    ...
    if (!roomKey) {
      setOnlineSession(current => ({ ...current, message: 'Enter a room key first.' }));
      return;
    }
  ...
  const setGuestReady = () => {
    ...
    setOnlineSession(current => ({ ...current, guestReady: true, message: 'Ready sent. Waiting for the host to start.' }));
  };
  ...
  useEffect(() => {
    ...
    const label = gameState.phase === 'NN_MOVE' ? 'Movement Step' : gameState.phase === 'NN_SELECT_CARD' ? 'Lock In Card' : 'Turn Start';
  ...
  const finalizeNNRound = () => {
    setGameState(prev => {
      ...
      const totalCardsDrawn = newPlayers.reduce((sum, player) => sum + (player.cardsDrawnLastCleanup || 0), 0);
      ...
      return {
         ...
         drawAnimation: totalCardsDrawn > 0 ? { key: Date.now(), playerId: 'both', count: totalCardsDrawn } : null,
         isNNRound: isNextNN
      };
    });
  };
    ...
    setGameState(prev => {
      ...
      if (isNNNow) {
        ...
        return {
          ...
          nnActiveTurnIdx: null,
          nnMpLeft: null,
          drawAnimation: cardsDrawn > 0 ? { key: Date.now(), playerId: p.id, count: cardsDrawn } : null,
          isNNRound: true
        };
      }
      ...
      <div className={`fixed ${meterPositionClass} z-40 flex w-[340px] flex-col bg-zinc-950/75 backdrop-blur-md p-3 rounded-2xl border border-white/15 shadow-2xl no-pan pointer-events-none ${isTutorialTarget ? 'tutorial-highlight' : ''}`}>
        ...
        <div className="relative aspect-square w-full overflow-visible">
          ...
          <div className="absolute left-1/2 top-[40%] -translate-x-1/2 -translate-y-1/2 text-center">
            <div className="text-sm font-black text-zinc-300/90 drop-shadow-[0_2px_8px_rgba(0,0,0,0.95)]">Speed</div>
          ...
          <div className="absolute right-[8%] bottom-[8%] flex items-baseline gap-1 text-right drop-shadow-[0_3px_10px_rgba(0,0,0,0.95)]">
            <span className="text-lg font-black text-zinc-300">Gear</span>
  ...
  const PileWidget = ({ type, count, topCard, onClick }) => {
    ...
    return (
      <button
        ...
        className={`relative h-44 w-32 rounded-xl border-2 p-2 text-left shadow-2xl transition no-pan ${isDiscard ? 'border-orange-500/70 bg-zinc-900 hover:border-orange-300 hover:-translate-y-1 cursor-pointer' : 'border-zinc-600 bg-zinc-950 cursor-not-allowed'}`}
      >
        ...
        <div className="mt-2 h-32 rounded-lg border border-white/15 overflow-hidden">
          {isDiscard && topCard ? (
            topCard.imageSrc ? (
    ...
    return (
      <div key={event.key} className="fixed bottom-24 right-24 z-[95] pointer-events-none no-pan">
        {Array.from({ length: cardCount }).map((_, index) => (
          <div
            key={index}
            className="absolute h-24 w-16 rounded-lg border-2 border-yellow-200 bg-zinc-950 shadow-[0_0_18px_rgba(250,204,21,0.45)]"
            style={{
              animation: `deck-card-draw 0.82s ${index * 0.08}s cubic-bezier(0.22, 1, 0.36, 1) forwards`,
          ...
          >
            <div className="flex h-full w-full flex-col items-center justify-center rounded-md bg-[repeating-linear-gradient(135deg,rgba(255,255,255,0.1)_0px,rgba(255,255,255,0.1)_6px,transparent_6px,transparent_12px)]">
              ...
              <div className="mt-3 text-[8px] font-black tracking-[0.2em] text-yellow-100/70">DRAW</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="relative flex h-screen w-full overflow-hidden text-white flex-col items-center justify-center bg-[#050505]">
        ...
        <div className="flex flex-wrap justify-center gap-4">
          ...
          <button onClick={() => beginSetup('PvP')} className="group relative px-8 py-4 bg-zinc-900 border-2 border-zinc-700 rounded-xl hover:border-blue-500 hover:bg-zinc-800 transition-all shadow-xl overflow-hidden">
            ...
            <div className="relative flex items-center gap-3 text-xl font-bold"><Users className="text-zinc-400 group-hover:text-blue-400" /><span>Local 2 Player</span></div>
          </button>
          <button onClick={openOnlineMatchmaking} className="group relative px-8 py-4 bg-zinc-900 border-2 border-zinc-700 rounded-xl hover:border-emerald-400 hover:bg-zinc-800 transition-all shadow-xl overflow-hidden">
            ...
            <div className="relative flex items-center gap-3 text-xl font-bold"><Radio className="text-zinc-400 group-hover:text-emerald-300" /><span>Online 2 Player</span></div>
          </button>
        </div>
        <button onClick={openCodex} className="group relative mt-5 min-w-64 overflow-hidden rounded-xl border-2 border-zinc-700 bg-zinc-950/90 px-8 py-3 shadow-xl transition-all hover:border-yellow-400 hover:bg-zinc-900">
          ...
          <div className="relative flex items-center justify-center gap-3 text-lg font-black uppercase tracking-wide"><BookOpen className="text-zinc-400 group-hover:text-yellow-300" /><span>Codex</span></div>
        </button>
        </div>
      ...
      <div className="h-screen w-full overflow-y-auto bg-[#060808] text-white">
        <div className="flex min-h-screen flex-col bg-[radial-gradient(circle_at_48%_18%,rgba(16,185,129,0.2),transparent_36%),linear-gradient(125deg,rgba(127,29,29,0.18),transparent_34%,rgba(30,64,175,0.18))] px-4 py-5 sm:px-8">
          <header className="mx-auto flex w-full max-w-6xl items-center justify-between border-b border-white/15 pb-4">
            <button onClick={() => setAppState(gameMode === 'OnlinePvP' ? 'MATCHMAKING' : 'TITLE')} className="flex items-center gap-1 text-xs font-black uppercase tracking-[0.2em] text-zinc-400 hover:text-white"><ChevronLeft size={18}/> {gameMode === 'OnlinePvP' ? 'Matchmaking' : 'Mode Select'}</button>
            <div className="text-center">
              <div className="text-3xl font-black italic uppercase tracking-tight sm:text-5xl">Online 2 Player</div>
              <div className="text-[10px] font-bold uppercase tracking-[0.35em] text-emerald-300">Room Matchmaking</div>
            </div>
            <div className="hidden rounded border border-emerald-400/30 bg-emerald-950/40 px-3 py-2 text-xs font-black uppercase text-emerald-200 sm:block">{onlineSession.role || 'Choose Role'}</div>
          </header>
          ...
          <main className="mx-auto grid w-full max-w-6xl flex-1 items-stretch gap-4 py-6 lg:grid-cols-2">
            <section className={`border bg-black/68 p-5 shadow-2xl transition ${onlineSession.role === 'host' ? 'border-emerald-300 shadow-[0_0_28px_rgba(16,185,129,0.18)]' : 'border-white/15'}`}>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="text-[10px] font-black uppercase tracking-[0.32em] text-emerald-300">Host</div>
                  <h2 className="mt-2 text-3xl font-black italic uppercase">Create Room</h2>
                </div>
                <Radio className="text-emerald-300" size={34}/>
              </div>
              <p className="mt-4 text-sm font-bold leading-relaxed text-zinc-400">Generate a room key, wait for the guest to connect, then start once their ready light is on.</p>
              <button onClick={createOnlineRoom} className="mt-6 w-full border-2 border-emerald-300 bg-emerald-400 px-5 py-3 text-base font-black uppercase text-black transition hover:bg-emerald-300">Host A Room</button>

              <div className="mt-6 border border-white/10 bg-zinc-950/80 p-4">
                <div className="text-[10px] font-black uppercase tracking-[0.28em] text-zinc-500">Room Key</div>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                <div className={`border px-4 py-3 ${onlineSession.guestConnected ? 'border-emerald-400 bg-emerald-400/10 text-emerald-200' : 'border-white/15 bg-black/45 text-zinc-500'}`}>
                  <div className="text-[10px] font-black uppercase tracking-widest">Guest</div>
                  <div className="mt-1 text-sm font-black uppercase">{onlineSession.guestConnected ? 'Connected' : 'Waiting'}</div>
                </div>
                <div className={`border px-4 py-3 ${onlineSession.guestReady ? 'border-yellow-300 bg-yellow-300/10 text-yellow-200' : 'border-white/15 bg-black/45 text-zinc-500'}`}>
                  <div className="text-[10px] font-black uppercase tracking-widest">Ready</div>
                  <div className="mt-1 text-sm font-black uppercase">{onlineSession.guestReady ? 'Locked In' : 'Not Yet'}</div>
                </div>
              </div>

              <button onClick={startOnlineSelection} disabled={!canHostStart} className={`mt-6 flex w-full items-center justify-center gap-3 border-2 px-5 py-4 text-lg font-black italic uppercase transition ${canHostStart ? 'border-yellow-300 bg-yellow-400 text-black hover:bg-yellow-300' : 'cursor-not-allowed border-white/10 bg-zinc-900 text-zinc-600'}`}>Start Setup <Play size={20}/></button>
            </section>
            ...
            <section className={`border bg-black/68 p-5 shadow-2xl transition ${onlineSession.role === 'guest' ? 'border-blue-300 shadow-[0_0_28px_rgba(59,130,246,0.18)]' : 'border-white/15'}`}>
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="text-[10px] font-black uppercase tracking-[0.32em] text-blue-300">Guest</div>
                  <h2 className="mt-2 text-3xl font-black italic uppercase">Join Room</h2>
                </div>
                <Users className="text-blue-300" size={34}/>
              </div>
              <p className="mt-4 text-sm font-bold leading-relaxed text-zinc-400">Enter the host room key. Once connected, mark ready and wait for the host to launch setup.</p>
              <div className="mt-6">
                <label className="text-[10px] font-black uppercase tracking-[0.28em] text-zinc-500">Room Key</label>
                <input
              ...
              <button onClick={joinOnlineRoom} disabled={!cleanGuestInput} className={`mt-4 w-full border-2 px-5 py-3 text-base font-black uppercase transition ${cleanGuestInput ? 'border-blue-300 bg-blue-400 text-black hover:bg-blue-300' : 'cursor-not-allowed border-white/10 bg-zinc-900 text-zinc-600'}`}>Join As Guest</button>
              ...
              <div className="mt-6 grid grid-cols-2 gap-3">
                <div className={`border px-4 py-3 ${onlineSession.role === 'guest' && onlineSession.guestConnected ? 'border-blue-300 bg-blue-400/10 text-blue-100' : 'border-white/15 bg-black/45 text-zinc-500'}`}>
                  <div className="text-[10px] font-black uppercase tracking-widest">Host</div>
                  <div className="mt-1 text-sm font-black uppercase">{onlineSession.role === 'guest' && onlineSession.guestConnected ? 'Found' : 'Searching'}</div>
                </div>
                <div className={`border px-4 py-3 ${onlineSession.role === 'guest' && onlineSession.guestReady ? 'border-yellow-300 bg-yellow-300/10 text-yellow-200' : 'border-white/15 bg-black/45 text-zinc-500'}`}>
                  <div className="text-[10px] font-black uppercase tracking-widest">Ready</div>
                  <div className="mt-1 text-sm font-black uppercase">{onlineSession.role === 'guest' && onlineSession.guestReady ? 'Sent' : 'Open'}</div>
                </div>
              </div>

              <button onClick={setGuestReady} disabled={onlineSession.role !== 'guest' || !onlineSession.guestConnected || onlineSession.guestReady} className={`mt-6 w-full border-2 px-5 py-4 text-lg font-black italic uppercase transition ${onlineSession.role === 'guest' && onlineSession.guestConnected && !onlineSession.guestReady ? 'border-yellow-300 bg-yellow-400 text-black hover:bg-yellow-300' : 'cursor-not-allowed border-white/10 bg-zinc-900 text-zinc-600'}`}>Ready</button>
            </section>
          </main>
  ...
  if (appState === 'CODEX') {
    ...
    const renderCodexGridCard = (card) => {
      ...
      const typeTone = card.type === 'Gas' ? 'border-yellow-400/70' : card.type === 'Brake' ? 'border-red-400/70' : card.type === 'Attack' ? 'border-orange-400/70' : 'border-blue-400/70';
      ...
      <div onClick={() => setCodexLockedId(null)} className="h-screen w-full overflow-hidden bg-[#070807] text-white">
        <div className="flex h-full flex-col bg-[radial-gradient(circle_at_70%_15%,rgba(250,204,21,0.12),transparent_34%),linear-gradient(135deg,rgba(127,29,29,0.16),transparent_38%,rgba(24,24,27,0.72))]">
          <header className="flex items-center justify-between border-b border-white/15 px-5 py-4 sm:px-8">
            <button onClick={() => setAppState('TITLE')} className="flex items-center gap-1 text-xs font-black uppercase tracking-[0.2em] text-zinc-400 hover:text-white"><ChevronLeft size={18}/> Title</button>
            <div className="text-center">
              <div className="text-3xl font-black italic tracking-tight sm:text-4xl">CODEX</div>
              <div className="text-[10px] font-bold uppercase tracking-[0.35em] text-yellow-400">Card Archive</div>
            </div>
            <div className="hidden rounded border border-white/20 bg-black/40 px-3 py-2 text-xs font-black uppercase text-zinc-300 sm:block">{activeList.length} Cards</div>
          </header>
          ...
          <main className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[22rem_1fr]">
            <aside className="min-h-0 overflow-y-auto border-b border-white/15 bg-black/65 px-4 py-5 lg:border-b-0 lg:border-r lg:px-5" onClick={() => setCodexLockedId(null)}>
              {activeItem && (
                <div>
                  ...
                  <div className="mt-7">
                    <div className="text-[10px] font-black uppercase tracking-[0.35em] text-yellow-400">{codexTab === 'racing' ? activeItem.deckCode : 'Road Card'}</div>
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-2">
                      {codexTab === 'racing' ? (
                        ...
                        <>
                          <span className="border border-white/15 bg-black/50 px-3 py-2 text-xs font-black uppercase text-zinc-300">Entry {activeItem.entrySpaces}</span>
                          <span className="border border-white/15 bg-black/50 px-3 py-2 text-xs font-black uppercase text-zinc-300">Exit {activeItem.exitSpaces}</span>
                          <span className="border border-white/15 bg-black/50 px-3 py-2 text-xs font-black uppercase text-zinc-300">Weight {activeItem.weight}</span>
                        </>
                      )}
                    </div>
                    ...
                    {codexTab === 'racing' ? (
                      <>
                        <div className="mt-6 border-l-4 border-yellow-400 bg-black/45 p-4">
                          <div className="text-[10px] font-black uppercase tracking-[0.28em] text-zinc-500">Effect</div>
                      ...
                      <div className="mt-6 space-y-3">
                        {activeItem.segments.map((segment, index) => (
                          <div key={index} className="border border-white/10 bg-black/45 p-4">
                            <div className="text-[10px] font-black uppercase tracking-[0.28em] text-zinc-500">Segment {index + 1}</div>
                            <div className="mt-2 text-base font-black uppercase text-white">{segment.type === 'straight' ? `Straight / ${segment.spaces} spaces` : `${segment.angle} degree ${segment.direction} curve`}</div>
                            {segment.type === 'curve' && <div className="mt-1 text-xs font-bold text-zinc-400">Radius {segment.radius} / Inner {segment.in_spaces} spaces / Outer {segment.out_spaces} spaces / Speed limit {segment.speed_limit} km/h</div>}
                          </div>
                        ))}
                      </div>
                    )}

                    {codexLockedId && <div className="mt-6 text-[10px] font-black uppercase tracking-[0.25em] text-yellow-400">Selection locked</div>}
                  </div>
                </div>
              )}
            </aside>
            ...
            <section className="flex min-h-0 flex-col bg-black/30" onClick={event => event.stopPropagation()}>
              <div className="grid grid-cols-2 gap-2 border-b border-white/15 bg-zinc-950/65 p-3">
                {[
                  ['racing', 'Racing Cards'],
                  ['road', 'Road Cards']
                ].map(([tab, label]) => (
        ...
        <div className="min-h-screen bg-[radial-gradient(circle_at_50%_20%,rgba(91,108,59,0.32),transparent_45%),linear-gradient(120deg,rgba(127,29,29,0.16),transparent_38%,rgba(30,58,138,0.16))] px-4 py-4 sm:px-6 lg:px-10">
          <header className="mx-auto flex max-w-7xl items-center justify-between border-b border-white/15 pb-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="rounded border border-white/20 bg-black/40 px-3 py-2 text-xs font-black uppercase text-zinc-300">{gameMode === 'PvE' ? '1P vs CPU' : gameMode === 'OnlinePvP' ? 'Online 2P' : 'Local 2P'}</div>
            </div>
          </header>
              ...
              {[0, 1].map(player => {
                ...
                return <button type="button" key={player} onClick={() => !(player === 1 && gameMode === 'PvE') && !lockedByOnlineRole && setActiveCarPlayer(player)} className={`overflow-hidden border bg-black/65 text-left ${player === 1 ? 'lg:col-start-3' : ''} ${player === 1 && gameMode === 'PvE' || lockedByOnlineRole ? 'cursor-default' : 'cursor-pointer hover:border-white/50'} ${waiting ? player === 0 ? 'border-red-500 shadow-[0_0_20px_rgba(239,68,68,0.25)]' : 'border-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.25)]' : 'border-white/15'}`}>
                  ...
                  <div className={`p-4 ${player === 1 ? 'text-right' : ''}`}>
                    ...
                    <p className="mt-2 min-h-10 text-xs leading-relaxed text-zinc-400">{car?.specialty || (player === 1 && gameMode === 'PvE' ? 'CPU machine locked.' : lockedByOnlineRole ? 'Remote player machine slot.' : 'Select from machine grid.')}</p>
                    {car && !(player === 1 && gameMode === 'PvE') && !lockedByOnlineRole && <div className="mt-2 text-[10px] font-black uppercase tracking-widest text-zinc-500">Press panel, then press any machine</div>}
                  </div>
                </button>;
              })}
              ...
              <div className="grid grid-cols-2 gap-2 self-stretch lg:col-start-2 lg:row-start-1">
                {CAR_OPTIONS.map((car, index) => {
                  ...
                  return <button key={car.id} onClick={() => chooseCar(car.id)} className={`group relative min-h-36 overflow-hidden border bg-zinc-950 p-3 text-left transition hover:-translate-y-1 hover:border-yellow-400 ${pickedBy.length ? 'border-yellow-500/60' : 'border-white/15'}`}>
                    ...
                    <div className="absolute -right-3 -top-4 text-7xl font-black italic text-white/[0.07]">0{index + 1}</div><div className="relative text-[10px] font-black uppercase tracking-widest text-zinc-400">{car.drive} / Machine 0{index + 1}</div><div className="relative mt-4 text-lg font-black uppercase leading-tight drop-shadow-[0_2px_0_rgba(0,0,0,0.8)]">{car.name}</div>
                ...
                <div className="col-span-2 flex min-h-14 items-center justify-center">
                  {canStartRaceSetup ? <button onClick={initGame} className="group flex items-center gap-3 border-2 border-yellow-300 bg-yellow-400 px-8 py-3 text-lg font-black italic uppercase text-black shadow-[0_0_26px_rgba(250,204,21,0.4)] transition hover:scale-105 hover:bg-yellow-300">Start Your Engines <Play size={20}/></button> : <div className="text-xs font-black uppercase tracking-[0.25em] text-zinc-500">{gameMode === 'OnlinePvP' && onlineRole === 'guest' ? 'Waiting for host to start' : activePicker === 0 ? 'Player 1: choose machine' : 'Player 2: choose machine'}</div>}
                </div>
              </div>
            </section>
          </main>
  ...
  return (
    ...
    >
      ...
      <style>{`
        ...
        @keyframes deck-card-draw {
          0% { opacity: 0; transform: translate(0, 0) scale(0.78) rotate(-10deg); }
          18% { opacity: 1; }
          100% { opacity: 0; transform: translate(-46vw, -20vh) scale(1.08) rotate(8deg); }
        }
        .tutorial-highlight {
      ...
      <TutorialOverlay step={activeTutorialStep} onDismiss={dismissTutorial} onSkip={skipTutorial} />
      ...
      {/* Fanned Cards UI */}
      {activeHandVisible && (
        <div className={`absolute ${activeHandTucked ? '-bottom-28' : 'bottom-8'} left-[40%] -translate-x-1/2 flex items-end justify-center z-40 w-full no-pan transition-all duration-300`} style={{ perspective: '1200px' }}>
          ...
          {activeHandCanPlay && <button onClick={skipToDiscard} className="absolute -bottom-2 right-[25%] px-9 py-6 bg-red-950/80 hover:bg-red-900 border border-red-900 rounded-lg text-m font-bold text-red-200 transition-colors shadow-2xl">Skip/Refresh</button>}
        </div>
      )}
