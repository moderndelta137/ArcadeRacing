...
const RoadCardPreview = ({ card }) => {
  ...
  const speedLimits = segments.filter(segment => segment.type === 'curve').map(segment => segment.speed_limit);
  const minLimit = speedLimits.length ? Math.min(...speedLimits) : null;
  const hasStraight = segments.some(segment => segment.type === 'straight');

  return (
    <div className="relative h-64 w-[11.5rem] overflow-hidden rounded-xl border-2 border-zinc-500 bg-[#10130d] p-4 shadow-2xl">
      ...
      <div className="relative flex h-full flex-col">
        <div className="flex items-center justify-between">
          <span className="text-[10px] font-black uppercase tracking-[0.24em] text-yellow-400">Road Card</span>
          <span className="rounded border border-white/20 bg-black/50 px-2 py-1 text-[10px] font-black text-zinc-300">W{card.weight}</span>
        </div>
        <div className="mt-5 flex flex-1 items-center justify-center">
          <svg viewBox="0 0 160 120" className="h-32 w-full drop-shadow-[0_0_18px_rgba(250,204,21,0.18)]" aria-hidden="true">
            <path d="M22 96 C42 72 42 48 66 42 S113 47 128 18" fill="none" stroke="#27272a" strokeWidth="28" strokeLinecap="round" />
            <path d="M22 96 C42 72 42 48 66 42 S113 47 128 18" fill="none" stroke="#f4f4f5" strokeWidth="8" strokeLinecap="round" />
            <path d="M22 96 C42 72 42 48 66 42 S113 47 128 18" fill="none" stroke="#facc15" strokeWidth="2" strokeLinecap="round" strokeDasharray="5 8" />
            <circle cx="22" cy="96" r="6" fill="#22c55e" stroke="white" strokeWidth="2" />
            <circle cx="128" cy="18" r="6" fill="#ef4444" stroke="white" strokeWidth="2" />
          </svg>
        </div>
        <div className="text-2xl font-black uppercase leading-none text-white drop-shadow-[0_0_10px_rgba(250,204,21,0.22)]">{card.name}</div>
        <div className="mt-3 grid grid-cols-3 gap-2 text-center">
          <div className="border border-white/10 bg-black/45 p-2">
            <div className="text-[9px] font-black uppercase tracking-widest text-zinc-500">Entry</div>
            <div className="text-lg font-black text-white">{card.entrySpaces}</div>
          </div>
          <div className="border border-white/10 bg-black/45 p-2">
            <div className="text-[9px] font-black uppercase tracking-widest text-zinc-500">Exit</div>
            <div className="text-lg font-black text-white">{card.exitSpaces}</div>
          </div>
          <div className="border border-white/10 bg-black/45 p-2">
            <div className="text-[9px] font-black uppercase tracking-widest text-zinc-500">Limit</div>
            <div className="text-lg font-black text-yellow-300">{minLimit || (hasStraight ? 'STR' : '--')}</div>
          </div>
        </div>
      </div>
    </div>
...
export default function ArcadeRacingGame() {
  ...
  if (appState === 'CODEX') {
    ...
    const renderCodexGridCard = (card) => {
      ...
      const speedLimits = card.segments?.filter(segment => segment.type === 'curve').map(segment => segment.speed_limit) || [];
      const minLimit = speedLimits.length ? Math.min(...speedLimits) : null;
        ...
        >
          {codexTab === 'racing' && card.imageSrc ? (
            ...
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_14%,rgba(250,204,21,0.22),transparent_34%),linear-gradient(145deg,rgba(39,39,42,0.95),rgba(5,5,5,1))]">
              ...
              <svg viewBox="0 0 120 92" className="absolute left-2 right-2 top-7 h-[46%] w-[calc(100%-1rem)]" aria-hidden="true">
                <path d="M16 76 C31 56 30 39 49 34 S85 36 99 13" fill="none" stroke="#27272a" strokeWidth="20" strokeLinecap="round" />
                <path d="M16 76 C31 56 30 39 49 34 S85 36 99 13" fill="none" stroke="#e4e4e7" strokeWidth="6" strokeLinecap="round" />
                <path d="M16 76 C31 56 30 39 49 34 S85 36 99 13" fill="none" stroke="#facc15" strokeWidth="1.5" strokeLinecap="round" strokeDasharray="4 6" />
              </svg>
              <div className="absolute left-2 right-2 top-2 flex justify-between text-[8px] font-black uppercase tracking-widest text-yellow-300">
                <span>Road</span>
                <span>{minLimit ? `${minLimit}` : 'STR'}</span>
              </div>
              <div className="absolute inset-x-2 bottom-2 text-left">
                <div className="text-[10px] font-black uppercase leading-tight text-white">{card.name}</div>
                <div className="mt-1 text-[8px] font-black uppercase tracking-widest text-zinc-400">E{card.entrySpaces} / X{card.exitSpaces}</div>
              </div>
            </div>
          )}
          {codexTab === 'racing' && !card.imageSrc && (
