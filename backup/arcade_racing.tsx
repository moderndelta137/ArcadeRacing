...
export default function ArcadeRacingGame() {
  ...
  const Speedometer = ({ player, position, projectedSpeed, isTurn, phase, mpLeft, isLeadStatus }) => {
    ...
    const meterPositionClass = isBottomLeft ? 'bottom-24 left-4' : 'top-12 right-4';
    const gearAccent = GEAR_ACCENT_COLORS[gearInfo.gear] || '#ffffff';

    return (
      <div className={`fixed ${meterPositionClass} z-40 flex w-72 flex-col bg-zinc-950/75 backdrop-blur-md p-3 rounded-2xl border border-white/15 shadow-2xl no-pan pointer-events-none`}>

        {/* Lead / Following Badge */}
        <div className={`absolute -top-3 right-4 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg border ${isLeadStatus ? 'bg-yellow-400 text-black border-yellow-500 whitespace-nowrap' : 'bg-zinc-800 text-zinc-400 border-zinc-600 whitespace-nowrap'}`}>
        ...
        <div className="mb-1 flex items-end justify-between gap-3 pr-20">
          ...
          <div className="font-mono text-xs font-black text-zinc-500">HUD</div>
        </div>
        ...
        <div className="relative aspect-square w-full overflow-visible">
          ...
          <div className="absolute left-1/2 top-[47%] -translate-x-1/2 -translate-y-1/2 text-center">
            ...
            <div className="font-mono text-5xl font-black leading-none text-white drop-shadow-[0_4px_12px_rgba(0,0,0,0.95)]">{player.speed}</div>
          </div>

          <div className="absolute right-[13%] bottom-[19%] flex items-baseline gap-1 text-right drop-shadow-[0_3px_10px_rgba(0,0,0,0.95)]">
          ...
          <div className="absolute left-1/2 bottom-[8%] grid h-16 w-20 -translate-x-1/2 place-items-center rounded-xl border-2 border-emerald-400/40 bg-emerald-950/80 shadow-[0_0_18px_rgba(16,185,129,0.28)]">
    ...
    >
      ...
      <svg width="100%" height="100%" className="absolute inset-0 z-0 bg-[#1e2418]">
        ...
        <g transform={`translate(${winSize.w/2 - 140}, ${winSize.h/2}) scale(${zoom}) translate(${-cameraTarget.x + panOffset.x}, ${-cameraTarget.y + panOffset.y})`} style={{ transition: isDragging ? 'none' : 'transform 0.3s cubic-bezier(0.25, 1, 0.5, 1)' }}>
          ...
          <image
            href="Assets/Backgrounds/snow_field_topdown_06.png"
